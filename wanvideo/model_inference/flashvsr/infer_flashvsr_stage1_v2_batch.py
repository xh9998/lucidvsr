...
def list_input_videos(input_dir: str):
    ...
    for name in os.listdir(input_dir):
        if name.lower().endswith(".mp4"):
            names.append(name)
    return [os.path.join(input_dir, name) for name in sorted(names)]


def main():
    parser = argparse.ArgumentParser(
        description="Batch inference for V2 Stage1 LoRA checkpoints. Loads Wan/VAE/projector once per checkpoint."
    )
