...
from wanvideo.model_training.flashvsr.train_flashvsr_stage1_v3 import (
    WanFixedPromptFlashVSRStage1Pipeline,
    flashvsr_stage1_v3_export,
    split_v3_exported_state,
)
...
def main():
    parser = argparse.ArgumentParser(description="Use V3 full-finetune checkpoint with V2-aligned fixed-prompt FlashVSR inference.")
    ...
    exported_state = flashvsr_stage1_v3_export(ckpt)
    dit_state, lq_proj_state, other_state = split_v3_exported_state(exported_state)
    effective_lq_proj_layer_num = args.lq_proj_layer_num or infer_lq_proj_layer_num(lq_proj_state)

    print(f"checkpoint={args.checkpoint_path}")
    print(f"dit_keys={len(dit_state)} lq_proj_keys={len(lq_proj_state)} other_keys={len(other_state)}")
    print(f"lq_proj_layer_num={effective_lq_proj_layer_num}")
    if other_state:
        print("warning: ckpt contains extra non-dit/non-projection keys.")
    ...
    dit_missing, dit_unexpected = pipe.dit.load_state_dict(dit_state, strict=False)
    proj_missing, proj_unexpected = pipe.lq_proj_in.load_state_dict(lq_proj_state, strict=False)
    if dit_missing:
        print(f"warning: missing dit keys: {len(dit_missing)}")
    if dit_unexpected:
        print(f"warning: unexpected dit keys: {len(dit_unexpected)}")
    if proj_missing:
        print(f"warning: missing lq_proj keys: {len(proj_missing)}")
    if proj_unexpected:
        print(f"warning: unexpected lq_proj keys: {len(proj_unexpected)}")
