...
CKPT_DIR="${CKPT_DIR:-/mnt/task_wrapper/user_output/artifacts/ckpts/stage3_v7_d1_step500}"
CKPT_S3_DIR="${CKPT_S3_DIR:-}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/task_wrapper/user_output/artifacts/inference/stage3_v7_d1_scan89_step500}"
S3_OUTPUT_DIR="${S3_OUTPUT_DIR:-s3://lxh/tmp/stage3_v7_d1_scan89_step500}"
...
pkill -f infer_flashvsr_stage3_v7_d_batch || true
...
ckpt_list="${OUTPUT_ROOT}/logs/stage3_v7_d1_step500_s3_ckpt_list.txt"
if [[ -n "${CKPT_S3_DIR}" ]]; then
...
while IFS= read -r file; do
  ...
  if [[ "${step}" =~ ^[0-9]+$ ]] && (( step >= MIN_STEP )) && (( step % STEP_MOD == 0 )); then
    if [[ ! -s "${CKPT_DIR}/${file}" ]]; then
      echo "[download] ${file}"
      if [[ -z "${CKPT_S3_DIR}" ]]; then
        echo "[error] ${file} missing locally and CKPT_S3_DIR is empty" >&2
        exit 1
      fi
