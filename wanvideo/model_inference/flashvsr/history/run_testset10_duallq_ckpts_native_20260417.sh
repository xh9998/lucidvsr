...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/task_wrapper/user_output/artifacts/inference/testset10_duallq_native_ckpts_20260417}"
...
run_one() {
  ...
  local kind="$3"
  local num_frames="$4"
  local testset_variant="$5"
  local ckpt_path="$6"

  local lq_dir="${TESTSET_ROOT}/${testset_variant}/lq_native"
  local run_dir="${OUTPUT_ROOT}/${tag}_${step}_${testset_variant}"
  mkdir -p "${run_dir}"
  : > "${run_dir}/run.log"
  ...
  for input_video in "${lq_dir}"/*.mp4; do
...

    {
      ...
      echo "TESTSET_VARIANT=${testset_variant}"
      echo "INPUT_VIDEO=${input_video}"
      echo "INPUT_KIND=native_lq"
