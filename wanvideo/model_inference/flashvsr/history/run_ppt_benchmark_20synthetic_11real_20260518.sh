...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/task_wrapper/user_output/artifacts/inference/ppt_benchmark_20synthetic_11real_20260518}"
SYNTHETIC_ROOT="${SYNTHETIC_ROOT:-/mnt/task_wrapper/user_output/artifacts/data/inference/testset20_89f_takano20250205_light_x4_lq_20260518}"
REAL_ROOT="${REAL_ROOT:-/mnt/task_wrapper/user_output/artifacts/data/inference/challenging_test_lxh_89f_320x192_resizecrop_20260503}"
SYNTHETIC_S3="${SYNTHETIC_S3:-s3://lxh/data/test/testset20_89f_takano20250205_light_x4_lq_20260518}"
REAL_S3="${REAL_S3:-s3://lxh/data/test/challenging_test_lxh_89f_320x192_resizecrop_20260503/}"
OUTPUT_S3="${OUTPUT_S3:-s3://lxh/data/test/ppt_benchmark_20synthetic_11real_20260518}"
...
mkdir -p "${OUTPUT_ROOT}/logs" "${CRITICAL_LOCAL}" "${OUTPUT_ROOT}/_inputs/synthetic_89f" "${OUTPUT_ROOT}/_inputs/real_89f"
cd "${ROOT_DIR}"
...
run_timed_job() {
  ...
  shift 5
  mkdir -p "${output_dir}"
  local start end status log_file
  log_file="${OUTPUT_ROOT}/logs/${method}_${dataset}.log"
  local time_file="${OUTPUT_ROOT}/logs/${method}_${dataset}.time"
  local num_in num_out old_status
  ...
  if [[ "${old_status}" == "0" && "${num_in}" != "0" && "${num_out}" == "${num_in}" ]]; then
    echo "[skip_done] gpu=${gpu} method=${method} dataset=${dataset} outputs=${num_out}/${num_in}"
    return 0
  fi
  echo "[job] gpu=${gpu} method=${method} dataset=${dataset} input=$(count_inputs "${input_dir}")"
  start="$(date +%s)"
  set +e
  ...
  record_time "${method}" "${dataset}" "$((end - start))" "${status}" "${input_dir}" "${output_dir}"
  if [[ "${status}" != "0" ]]; then
    echo "[job_failed] method=${method} dataset=${dataset} status=${status}" >&2
    return "${status}"
  fi
}

run_flashvsr_official() {
  local gpu="$1" dataset="$2" input_dir="$3" output_dir="$4"
  run_timed_job "${gpu}" flashvsr_official "${dataset}" "${input_dir}" "${output_dir}" \
...
run_seedvr() {
  local gpu="$1" method="$2" kind="$3" model_dir="$4" dataset="$5" input_dir="$6" output_dir="$7"
  run_timed_job "${gpu}" "${method}" "${dataset}" "${input_dir}" "${output_dir}" \
    env SEEDVR_PYTHON="${SEEDVR_PYTHON}" MODEL_KIND="${kind}" MODEL_DIR="${model_dir}" INPUT_DIR="${input_dir}" OUTPUT_DIR="${output_dir}" \
      ...
      LOG_FILE="${OUTPUT_ROOT}/logs/${method}_${dataset}_inner.log" bash "${ROOT_DIR}/wanvideo/model_inference/flashvsr/history/run_seedvr_dir_20260421.sh"
}

run_stage1() {
  local gpu="$1" method="$2" ckpt="$3" dataset="$4" input_dir="$5" output_dir="$6"
  run_timed_job "${gpu}" "${method}" "${dataset}" "${input_dir}" "${output_dir}" \
...
run_stage2_like() {
  local gpu="$1" method="$2" ckpt="$3" steps="$4" dataset="$5" input_dir="$6" output_dir="$7"
  run_timed_job "${gpu}" "${method}" "${dataset}" "${input_dir}" "${output_dir}" \
...
echo "[prepare] restoring datasets and critical checkpoints"
if [[ ! -d "${SYNTHETIC_ROOT}/lq" ]]; then
  conductor s3 sync "${SYNTHETIC_S3}" "${SYNTHETIC_ROOT}"
fi
if [[ ! -d "${REAL_ROOT}" ]]; then
  mkdir -p "${REAL_ROOT}"
  conductor s3 sync "${REAL_S3}" "${REAL_ROOT}"
fi
...
prepare_inputs "${REAL_ROOT}" "${OUTPUT_ROOT}/_inputs/real_89f"
mkdir -p "${OUTPUT_ROOT}/synthetic_89f" "${OUTPUT_ROOT}/real_89f"
rm -rf "${OUTPUT_ROOT}/synthetic_89f/gt" "${OUTPUT_ROOT}/synthetic_89f/lq" "${OUTPUT_ROOT}/real_89f/lq"
cp -a "${SYNTHETIC_ROOT}/gt" "${OUTPUT_ROOT}/synthetic_89f/gt"
cp -a "${SYNTHETIC_ROOT}/lq" "${OUTPUT_ROOT}/synthetic_89f/lq"
mkdir -p "${OUTPUT_ROOT}/real_89f/lq"
cp -a "${REAL_ROOT}/." "${OUTPUT_ROOT}/real_89f/lq/"

{
  echo "benchmark=ppt_benchmark_20synthetic_11real_20260518"
  echo "synthetic_root=${SYNTHETIC_ROOT}"
  echo "real_root=${REAL_ROOT}"
  echo "critical_s3=${CRITICAL_S3}"
  echo "output_s3=${OUTPUT_S3}"
  echo "gpu_rule=one GPU runs one method/dataset job; no stacked jobs on one GPU"
...
declare -a JOBS=(
  "flashvsr_official|synthetic_89f"
  "seedvr3b|synthetic_89f"
  "seedvr2_3b|synthetic_89f"
  "stage1_535_step10000|synthetic_89f"
  "stage1_usmgt_step3000|synthetic_89f"
  "stage2_v641_step6000|synthetic_89f"
  "stage3_v7d32_step2000|synthetic_89f"
  "flashvsr_official|real_89f"
  "seedvr3b|real_89f"
  "seedvr2_3b|real_89f"
  "stage1_535_step10000|real_89f"
  "stage1_usmgt_step3000|real_89f"
  "stage2_v641_step6000|real_89f"
  "stage3_v7d32_step2000|real_89f"
)
...
run_one_job() {
  ...
  local method="${job%%|*}"
  local dataset="${job##*|}"
  local input_dir="${OUTPUT_ROOT}/_inputs/${dataset}"
  local output_dir="${OUTPUT_ROOT}/${dataset}/${method}"
  case "${method}" in
    flashvsr_official) run_flashvsr_official "${gpu}" "${dataset}" "${input_dir}" "${output_dir}" ;;
    seedvr3b) run_seedvr "${gpu}" seedvr3b seedvr1 "${SEEDVR3B_MODEL_DIR}" "${dataset}" "${input_dir}" "${output_dir}" ;;
    seedvr2_3b) run_seedvr "${gpu}" seedvr2_3b seedvr2 "${SEEDVR2_3B_MODEL_DIR}" "${dataset}" "${input_dir}" "${output_dir}" ;;
    stage1_535_step10000) run_stage1 "${gpu}" stage1_535_step10000 "${STAGE1_535_CKPT}" "${dataset}" "${input_dir}" "${output_dir}" ;;
    stage1_usmgt_step3000) run_stage1 "${gpu}" stage1_usmgt_step3000 "${STAGE1_USMGT_CKPT}" "${dataset}" "${input_dir}" "${output_dir}" ;;
    stage2_v641_step6000) run_stage2_like "${gpu}" stage2_v641_step6000 "${STAGE2_641_CKPT}" 50 "${dataset}" "${input_dir}" "${output_dir}" ;;
    stage3_v7d32_step2000) run_stage3 "${gpu}" stage3_v7d32_step2000 "${STAGE3_D32_CKPT}" "${dataset}" "${input_dir}" "${output_dir}" ;;
...
if [[ "${START_OCCUPY_AFTER}" == "1" ]]; then
  echo "[occupy] restarting normal occupy on GPUS=${GPU_LIST}"
  pkill -f gpu_stress_tc.py || true
  pkill -f gpu_stress_tc.sh || true
  sleep 2
  tmux kill-session -t occupy_pptbench 2>/dev/null || true
  tmux new-session -d -s occupy_pptbench "PYTHON_BIN=/mnt/conda_envs/flashvsr/bin/python GPUS='${GPU_LIST}' bash /mnt/task_runtime/bolt_lxh/occupy/gpu_stress_tc.sh"
fi
