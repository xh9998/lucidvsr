...
run_seedvr2_dataset() {
  ...
  local out_dir="${OUTPUT_ROOT}/${dataset}/seedvr2_3b"
  local log_file="${OUTPUT_ROOT}/logs/seedvr2_3b_${dataset}.log"
  local start end status num_in num_out

  mkdir -p "${out_dir}"
  start="$(date +%s)"
  set +e
  CUDA_DEVICE="${gpu}" MODEL_KIND="seedvr2" MODEL_DIR="${SEEDVR2_3B_MODEL_DIR}" INPUT_DIR="${input_dir}" OUTPUT_DIR="${out_dir}" \
    SEEDVR_PYTHON="${SEEDVR_PYTHON}" RES_H=768 RES_W=1280 SEED="${SEED}" LOG_FILE="${log_file}" OUT_FPS="${FPS}" \
    MASTER_PORT="$((29620 + gpu))" \
    bash "${ROOT_DIR}/wanvideo/model_inference/flashvsr/history/run_seedvr_dir_20260421.sh" 2>&1 | tee "${OUTPUT_ROOT}/logs/seedvr2_3b_${dataset}_launcher.log"
  status="${PIPESTATUS[0]}"
  set -e
  end="$(date +%s)"
  ...
  if [[ "${status}" != "0" && "${num_out}" == "${num_in}" && "${num_in}" != "0" ]]; then
    echo "[warn] ${dataset}: SeedVR2 exited ${status}, but outputs are complete (${num_out}/${num_in}); accepting result." | tee -a "${OUTPUT_ROOT}/logs/seedvr2_3b_${dataset}_launcher.log"
    status=0
  fi
  record_time "seedvr2_3b" "${dataset}" "$((end - start))" "${status}" "${input_dir}" "${out_dir}"
  return "${status}"
}

{
  ...
  echo "SEEDVR2_3B_MODEL_DIR=${SEEDVR2_3B_MODEL_DIR}"
  echo "SYNTHETIC_GPU=${SYNTHETIC_GPU}"
  echo "REAL_GPU=${REAL_GPU}"
  echo "SeedVR2-3B: RES_H=768 RES_W=1280 OUT_FPS=${FPS}"
} | tee "${OUTPUT_ROOT}/logs/seedvr2_3b_89f_settings.txt"

run_seedvr2_dataset synthetic_89f "${SYNTHETIC_89_INPUT_DIR}" "${SYNTHETIC_GPU}" &
pid0=$!
run_seedvr2_dataset real_89f "${REAL_89_INPUT_DIR}" "${REAL_GPU}" &
pid1=$!
...
if [[ "${START_OCCUPY_AFTER}" == "1" ]]; then
  echo "[occupy] restarting gpu_stress_tc.sh"
  pkill -f gpu_stress_tc.py || true
  pkill -f gpu_stress_tc.sh || true
  sleep 2
  PYTHON_BIN=/mnt/conda_envs/flashvsr/bin/python GPUS="${SYNTHETIC_GPU},${REAL_GPU}" \
