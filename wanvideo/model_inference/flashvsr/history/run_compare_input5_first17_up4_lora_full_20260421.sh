...
OUTPUT_ROOT="${OUTPUT_ROOT:-/mnt/task_wrapper/user_output/artifacts/inference/compare_triplet_input5_first17_20260421}"
...
run_batch_v2() {
  local output_dir="${OUTPUT_ROOT}/lora17_step2500_alpha1"
  mkdir -p "${output_dir}"
  : > "${output_dir}/run.log"
...
run_batch_v3() {
  local output_dir="${OUTPUT_ROOT}/fullft17_step2800_alpha1"
  mkdir -p "${output_dir}"
  : > "${output_dir}/run.log"
  for input_video in "${INPUT_DIR}"/*.mp4; do
    local name
