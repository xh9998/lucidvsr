...
def _stage3d41_is_generator_turn(args, runner_step: int) -> bool:
    ...
    # D4.1 is physically turn-isolated: one generator turn is followed by
    # `ratio` fake-only turns. This keeps fake/student backward paths separate
    # under DeepSpeed while preserving DMD2's fake-updates-per-generator intent.
    return int(runner_step) % (ratio + 1) == 0
...
def launch_stage3c_dual_optimizer_task(
    ...
    if getattr(args, "resume_training_state_dir", None):
        ...
        runner_step = global_step * (dfake_ratio + 1)
    ...
    if accelerator.is_main_process:
        print(
            ...
            f"dfake_fake_updates_per_generator={_stage3d41_dfake_gen_update_ratio(args)} "
            "turn_isolated=1 "
    ...
    for epoch_id in range(resumed_epoch_id, num_epochs):
        ...
        while True:
            ...
            with accelerator.accumulate(model):
                ...
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                    timing_parts["backward"] = now - timing_last
                    timing_last = now
                ...
                if accelerator.is_main_process:
                    ...
                    if log_loss_steps and current_runner_step % log_loss_steps == 0:
                        ...
                        print(
                            ...
                            f"turn={'generator' if generator_turn else 'fake'} "
                            f"generator_update={int(generator_turn)} dfake_fake_updates_per_generator={_stage3d41_dfake_gen_update_ratio(args)} "
                    ...
                    if wandb_run is not None:
                        log_payload = {
                            ...
                            "train/dfake_fake_updates_per_generator": _stage3d41_dfake_gen_update_ratio(args),
...
def _stage3_parser():
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "v7-D4.1 turn-isolated DMD2-style dfake/gen ratio. One student/generator "
            "turn is followed by this many G_fake-only turns. Larger values mean more "
            "fake critic updates per student update."
        ),
    )
