...
def _stage3d43_load_deepspeed_config(args, *, micro_batch_size: int) -> Dict[str, Any]:
    """Build a concrete DeepSpeed config for the standalone G_fake engine."""
    ...
    if config_path is None:
        config_path = "/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/lora/history/deepspeed_zero2_flashvsr_noactckpt.json"
    if not os.path.exists(config_path):
        local_fallback = Path(__file__).resolve().parent / "lora" / "history" / "deepspeed_zero2_flashvsr_noactckpt.json"
        if local_fallback.exists():
            config_path = str(local_fallback)
    ...
    if not fake_offload:
        ...
        ds_config["_stage3_fake_ds_offload"] = False
    else:
        ds_config["_stage3_fake_ds_offload"] = True
    ds_config["zero_force_ds_cpu_optimizer"] = False
    return ds_config


def _stage3d43_init_fake_deepspeed_engine(accelerator, fake_model, fake_optimizer, fake_scheduler, args, *, micro_batch_size: int):
    try:
        import deepspeed
    except ImportError as error:
        raise RuntimeError("Stage3 v7-D4.3 requires deepspeed to initialize the standalone G_fake engine.") from error

    fake_model.to(accelerator.device)
    ds_config = _stage3d43_load_deepspeed_config(args, micro_batch_size=micro_batch_size)
    fake_engine, fake_optimizer, _, fake_scheduler = deepspeed.initialize(
        model=fake_model,
        model_parameters=[param for param in fake_model.parameters() if param.requires_grad],
        optimizer=fake_optimizer,
        lr_scheduler=fake_scheduler,
        config=ds_config,
        dist_init_required=False,
    )
    return fake_engine, fake_optimizer, fake_scheduler, ds_config
...
def _stage3d43_save_fake_deepspeed_state(accelerator, output_path: str, step: int, fake_engine, args) -> None:
    ...
    client_state = {
        ...
        "stage3_fake_fm_weight": float(getattr(args, "stage3_fake_fm_weight", 0.0)),
        "stage3_dfake_gen_update_ratio": int(_stage3d4_dfake_gen_update_ratio(args)),
        "note": "v7-D4.3 standalone DeepSpeed engine for trainable G_fake.",
    }
...
def launch_stage3c_dual_optimizer_task(
    ...
    fake_model, fake_optimizer, fake_scheduler, fake_ds_config = _stage3d43_init_fake_deepspeed_engine(
        accelerator,
        fake_model,
        fake_optimizer,
        fake_scheduler,
        args,
        micro_batch_size=dataloader_batch_size,
    )
    ...
    if getattr(args, "resume_training_state_dir", None):
        ...
        _stage3d43_load_fake_deepspeed_state_if_available(accelerator, args.resume_training_state_dir, fake_model)
        print(
    ...
    if accelerator.is_main_process:
        print(
            "[stage3c_runner] D4.3 dual DeepSpeed engine "
            ...
            "fake_backward=deepspeed_engine "
            ...
            f"fake_ds_offload={int(bool(fake_ds_config.get('_stage3_fake_ds_offload', False)))}",
            flush=True,
        )
        if real_model is not None:
            print(
