...
class FlashVSRStage3BTrainingModule(v6.FlashVSRStage2TrainingModule):
    ...
    def forward(self, data, inputs=None):
        ...
        merged_inputs[0]["lq_latent_alignment"] = _stage3d3_teacher_lq_alignment_mode(self.pipe)
...
def _stage3d3_param_checksum(model: Optional[torch.nn.Module], *, trainable_only: bool, max_params: int = 8) -> Optional[float]:
    if model is None:
        return None
    total = 0.0
    seen = 0
    with torch.no_grad():
        for param in model.parameters():
            if trainable_only and not param.requires_grad:
                continue
            if (not trainable_only) and param.requires_grad:
                continue
            if param.numel() == 0:
                continue
...
def _stage3d3_teacher_lq_alignment_mode(pipe) -> str:
...
def _stage3c_fake_fm_loss(
    ...
    *,
    ...
    if fake_weight == 0.0 or clean_latents is None or not isinstance(fake_model, FlashVSRStage3BTrainingModule):
        return None
    update_every = _stage3c_fake_update_every_n_steps(args)
    # This is explicitly "update G_fake every N student steps". It is not the
    # DMD2 dfake/gen ratio, where larger values mean more fake updates per
    # generator update.
...
def launch_stage3c_dual_optimizer_task(
    ...
    if accelerator.is_main_process:
        print(
            ...
            f"fake_update_every_n_steps={_stage3c_fake_update_every_n_steps(args)} "
    ...
    for epoch_id in range(resumed_epoch_id, num_epochs):
        ...
        while True:
            ...
            with accelerator.accumulate(model):
                ...
                ownership_debug = (
                    os.environ.get("FLASHVSR_STAGE3_OPT_OWNERSHIP_DEBUG", "0") == "1"
                    and int(global_step) == 0
                    and accelerator.is_main_process
                )
                if ownership_debug:
                    student_checksum_before = _stage3d3_param_checksum(_unwrap_stage3c_model(model), trainable_only=True)
                    fake_checksum_before = _stage3d3_param_checksum(fake_model, trainable_only=True)
                    real_checksum_before = _stage3d3_param_checksum(real_model, trainable_only=False)
                ...
                if fake_did_update:
                    fake_optimizer.step()
                if ownership_debug:
                    student_checksum_after = _stage3d3_param_checksum(_unwrap_stage3c_model(model), trainable_only=True)
                    fake_checksum_after = _stage3d3_param_checksum(fake_model, trainable_only=True)
                    real_checksum_after = _stage3d3_param_checksum(real_model, trainable_only=False)
                    print(
                        "[stage3d3_optimizer_ownership] "
                        f"student_changed={student_checksum_before != student_checksum_after} "
                        f"fake_changed={fake_checksum_before != fake_checksum_after} "
                        f"real_changed={real_checksum_before != real_checksum_after} "
                        f"student_before={student_checksum_before} student_after={student_checksum_after} "
                        f"fake_before={fake_checksum_before} fake_after={fake_checksum_after} "
                        f"real_before={real_checksum_before} real_after={real_checksum_after} "
                        f"fake_update={int(fake_did_update)}",
                        flush=True,
                    )
...
def _stage3_parser():
    ...
    parser.add_argument(
        ...
        help="v7-D3: LQ projector temporal mode for trainable G_fake. Use nonstreaming_aligned for Stage1 v5.3.5 teacher-copy semantics.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="nonstreaming_aligned",
        choices=("streaming", "nonstreaming", "nonstreaming_aligned"),
        help="v7-D3: LQ projector temporal mode for frozen G_real. Use nonstreaming_aligned for Stage1 v5.3.5 teacher semantics.",
    )
    parser.add_argument(
...
def parse_stage3_args(argv=None):
    ...
    if (float(getattr(args, "stage3_fake_fm_weight", 0.0)) > 0.0 or float(getattr(args, "stage3_dmd_weight", 0.0)) > 0.0) and not getattr(
        ...
        parser.error(
            "v7-D3 requires --stage3_fake_checkpoint when stage3_fake_fm_weight or stage3_dmd_weight is enabled; "
