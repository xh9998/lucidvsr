...
def _preprocess_video_samples(
    pipe,
    video: Union[torch.Tensor, Sequence[torch.Tensor]],
) -> Union[torch.Tensor, List[torch.Tensor]]:
    if torch.is_tensor(video):
        return pipe.preprocess_video(video)
    return [pipe.preprocess_video(sample) for sample in video]


def _pad_lq_layer_tokens_to_max(batch_layer_outputs: Sequence[torch.Tensor]) -> torch.Tensor:
    max_tokens = max(int(layer.shape[1]) for layer in batch_layer_outputs)
    padded_outputs: List[torch.Tensor] = []
    for layer in batch_layer_outputs:
        pad_tokens = max_tokens - int(layer.shape[1])
        if pad_tokens > 0:
            padding = torch.zeros(
                layer.shape[0],
                pad_tokens,
            ...
            layer = torch.cat([padding, layer], dim=1)
        padded_outputs.append(layer)
    return torch.cat(padded_outputs, dim=0)


def _pad_latent_time_to_max(encoded_samples: Sequence[torch.Tensor]) -> torch.Tensor:
    max_frames = max(int(sample.shape[2]) for sample in encoded_samples)
    padded_samples: List[torch.Tensor] = []
    for sample in encoded_samples:
        pad_frames = max_frames - int(sample.shape[2])
        if pad_frames > 0:
            padding = torch.zeros(
                sample.shape[0],
                sample.shape[1],
            ...
            sample = torch.cat([sample, padding], dim=2)
        padded_samples.append(sample)
    return torch.cat(padded_samples, dim=0)


def _encode_video_segments_with_vae(
    pipe,
    input_video: Union[torch.Tensor, Sequence[torch.Tensor]],
    *,
    ...
    use_sample_list = not torch.is_tensor(input_video)
    for sample_index, one_sample_lengths in enumerate(raw_segment_lengths):
        sample = input_video[sample_index] if use_sample_list else input_video[sample_index : sample_index + 1]
        offset = 0
    ...
    first_shape = tuple(encoded_samples[0].shape[1:])
    if not all(tuple(sample.shape[1:]) == first_shape for sample in encoded_samples):
        raise ValueError(
            f"Segment-aware VAE outputs are not aligned without padding: "
            f"{[list(sample.shape) for sample in encoded_samples]}"
        )
...
def _encode_lq_segments_with_projection(
    pipe,
    lq_video: Union[torch.Tensor, Sequence[torch.Tensor]],
    *,
    raw_segment_lengths: Optional[Sequence[Sequence[int]]],
    sample_kinds: Optional[Sequence[Any]] = None,
):
    if raw_segment_lengths is None:
        return pipe.lq_proj_in(lq_video)

    batch_outputs: Optional[List[List[torch.Tensor]]] = None
    use_sample_list = not torch.is_tensor(lq_video)
    for sample_index, one_sample_lengths in enumerate(raw_segment_lengths):
        sample = lq_video[sample_index] if use_sample_list else lq_video[sample_index : sample_index + 1]
        offset = 0
        per_segment_outputs: List[List[torch.Tensor]] = []
        sample_kind = None if sample_kinds is None else sample_kinds[sample_index]
        for seg_len in one_sample_lengths:
            segment = sample[:, :, offset : offset + seg_len]
            offset += seg_len
            if sample_kind == "image_group":
                encoded = pipe.lq_proj_in.forward_image_nocache(segment)
            else:
    ...
    for one_layer in batch_outputs:
        first_shape = tuple(one_layer[0].shape[1:])
        if not all(tuple(layer.shape[1:]) == first_shape for layer in one_layer):
            raise ValueError(
                f"Segment-aware LQ projection outputs are not aligned without padding: "
                f"{[list(layer.shape) for layer in one_layer]}"
            )
        outputs.append(torch.cat(one_layer, dim=0))
    return outputs
...
def _build_v5_sample_loss_weights(data: Dict[str, Any]) -> Optional[torch.Tensor]:
    ...
    if sample_kinds is None:
        return None
    image_group_sizes = data.get("image_group_size")
    if image_group_sizes is not None and torch.is_tensor(image_group_sizes):
        image_group_sizes_list = [int(v) for v in image_group_sizes.detach().cpu().tolist()]
    elif image_group_sizes is not None:
        image_group_sizes_list = [int(v) for v in image_group_sizes]
    else:
        image_group_sizes_list = [0] * len(sample_kinds)

    # V5.2 keeps the grouped-image data path, but reweights the grouped image
    # branch so that the whole grouped image sample is as heavy as an author-style
    # standalone image loss branch.
    #
    # Because the grouped sample internally averages over K independent images,
    # we multiply the grouped sample weight by K here.
    weights: List[float] = []
    for sample_kind, group_size in zip(sample_kinds, image_group_sizes_list):
        if sample_kind == "image_group":
            weights.append(float(max(1, int(group_size))))
        else:
            weights.append(1.0)
...
class FlashVSRLQProjIn(nn.Module):
    ...
    def forward_image_nocache(self, image_segment):
        b, c, t, h, w = image_segment.shape
        if t <= 0:
            raise ValueError(f"image_segment must have positive temporal length, got {tuple(image_segment.shape)}")
        # Image groups should stay on a true 2D path:
        # - collapse B and T into a single batch
        # - reuse the 3D kernels by reducing them over temporal dim
        # - avoid Python per-frame loops and fake temporal repetition
        frames = rearrange(image_segment, "b c t h w -> (b t) c 1 h w")
        x = self.pixel_shuffle(frames).squeeze(2)
        conv1_weight_2d = self.conv1.weight.sum(dim=2)
        conv2_weight_2d = self.conv2.weight.sum(dim=2)
        conv1_padding_2d = (self.conv1._padding[2], self.conv1._padding[0])
        conv2_padding_2d = (self.conv2._padding[2], self.conv2._padding[0])
        x = F.conv2d(
            x,
            conv1_weight_2d,
            bias=self.conv1.bias,
            stride=self.conv1.stride[1:],
            padding=conv1_padding_2d,
            dilation=self.conv1.dilation[1:],
        ...
        x = F.conv2d(
            ...
            padding=conv2_padding_2d,
            dilation=self.conv2.dilation[1:],
            groups=self.conv2.groups,
        )
        x = self.norm2(x)
        x = self.act2(x)
        x = rearrange(x, "(b t) c h w -> b (t h w) c", b=b, t=t)
        return [layer(x) for layer in self.linear_layers]
...
class WanVideoUnit_InputVideoEmbedderV5(PipelineUnit):
    ...
    def process(
        ...
        input_video = _preprocess_video_samples(pipe, input_video)
        raw_segment_lengths = _resolve_raw_segment_lengths(sequence_lengths, segment_lengths)
...
class FlashVSRUnit_LQVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("lq_video", "height", "width", "sequence_lengths", "segment_lengths", "sample_kind"),
            output_params=("lq_latents",),
            onload_model_names=("lq_proj_in",),
        )

    def process(self, pipe, lq_video, height, width, sequence_lengths=None, segment_lengths=None, sample_kind=None):
        if lq_video is None:
            return {}
        ...
        lq_video = _preprocess_video_samples(pipe, lq_video)
        _dump_tensor_preview_once(
            "02_preprocessed_lq_tensor",
            lq_video.permute(0, 2, 1, 3, 4).contiguous() if torch.is_tensor(lq_video) else lq_video[0].permute(0, 2, 1, 3, 4).contiguous(),
            pipe=pipe,
        )
        lq_input = lq_video
        raw_segment_lengths = _resolve_raw_segment_lengths(sequence_lengths, segment_lengths)
...
class FlashVSRStage1Pipeline(WanVideoPipeline):
    ...
    def from_pretrained(
        ...
        pipe.units = [
            ...
            WanVideoUnit_InputVideoEmbedderV5(),
            FlashVSRUnit_LQVideoEmbedder(),
        ]
        pipe.post_units = []
        pipe.in_iteration_models = ("dit",)
...
class FlashVSRStage1TrainingModule(DiffusionTrainingModule):
    ...
    def get_pipeline_inputs(self, data):
        if torch.is_tensor(data["video"]):
            ...
            if video.ndim == 5:
                ...
                if "target_num_frames" in data:
                    target_num_frames = data["target_num_frames"]
                    if torch.is_tensor(target_num_frames):
                        if target_num_frames.numel() != 1:
                            raise ValueError(
                                f"v5 training expects a scalar target_num_frames per batch item after collation, got {tuple(target_num_frames.shape)}"
                            )
                        num_frames = int(target_num_frames.item())
                    elif isinstance(target_num_frames, list):
                        if len(set(int(v) for v in target_num_frames)) != 1:
                            raise ValueError(f"Mixed target_num_frames in one batch: {target_num_frames}")
                        num_frames = int(target_num_frames[0])
                    else:
        ...
        raw_video = data.get("raw_video_list", video)
        raw_lq_video = data.get("raw_lq_video_list", data["lq_video"])
        inputs_shared = {
            "input_video": raw_video,
            "lq_video": raw_lq_video,
            "height": height,
            "width": width,
            "num_frames": num_frames,
            "cfg_scale": 1.0,
        ...
        if "sample_kind" in data:
            inputs_shared["sample_kind"] = data["sample_kind"]
        loss_sample_weights = _build_v5_sample_loss_weights(data)
        if loss_sample_weights is not None:
            inputs_shared["loss_sample_weights"] = loss_sample_weights
    ...
    def forward(self, data, inputs=None):
        ...
        loss = FlowMatchSFTLossV5(self.pipe, **inputs[0], **inputs[1])
        return loss
...
def flashvsr_parser():
    ...
    parser.add_argument("--dataset_mode", type=str, default="unified", choices=("unified", "streaming", "parquet_v2", "tar_v3", "tar_v5"), help="Dataset backend.")
    ...
    parser.add_argument("--picked17k_image_tar_url", type=str, default=None, help="picked17k image tar root for tar_v3/tar_v5 mode.")
    parser.add_argument("--picked17k_dataset_prob", type=float, default=0.1, help="Sampling probability of picked17k image source in tar_v3/tar_v5 mode.")
...
def main():
    ...
    if args.dataset_mode == "streaming":
        ...
        dataset = FlashVSRTarStreamingDatasetV3(
            yubari_video_tar_url=args.yubari_video_tar_url,
            picked17k_image_tar_url=args.picked17k_image_tar_url,
