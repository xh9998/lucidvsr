...
RUN_NAME="train_stage1_release_8gpu_v1_${RUN_TS}"
RUN_DIR="${EXP_ROOT}/${RUN_NAME}"
mkdir -p "${RUN_DIR}"
export FLASHVSR_DEBUG_DIR="${RUN_DIR}/debug"
mkdir -p "${FLASHVSR_DEBUG_DIR}"

TRAIN_PY="wanvideo/model_training/flashvsr/train_flashvsr_stage1_v1.py"
CONFIG_YAML="wanvideo/model_training/flashvsr/configs/stage1_release_8gpu_v1.yaml"
ACCEL_YAML="wanvideo/model_training/flashvsr/lora/accelerate_zero2_flashvsr_8gpu.yaml"
SELF_SH="wanvideo/model_training/flashvsr/lora/FlashVSR-Stage1-Release-8GPU-v1.sh"
...
cp "${TRAIN_PY}" "${RUN_DIR}/snapshot/"
cp "${CONFIG_YAML}" "${RUN_DIR}/snapshot/"
cp "${ACCEL_YAML}" "${RUN_DIR}/snapshot/"
cp "${SELF_SH}" "${RUN_DIR}/snapshot/"
