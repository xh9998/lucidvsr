...
export LQ_PROJ_CKPT="${LQ_PROJ_CKPT:-/mnt/models/FlashVSR-v1.1/LQ_proj_in.ckpt}"

CONFIG_PATH="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage1_release_16gpu_v5_3_2_phase2_unfreeze_from_step2000_seed20260426.yaml"
OUTPUT_TAG="train_stage1_release_16gpu_v5_3_2_phase2_unfreeze_from_step2000_seed20260426"
...
STAGE1_CKPT="/mnt/task_wrapper/user_output/artifacts/exp/train_stage1_release_16gpu_v5_3_2_lora_17f_yubari_frameimage_bs12_lr1e5_aliyundegra_flashinit_freezeproj_20260423_224600/output/step-2000.safetensors"
...
TRAIN_PY="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/train_flashvsr_stage1_v5_3_2_lora.py"
SELF_SH="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/lora/history/FlashVSR-Stage1-Release-16GPU-v5-3-2-Phase2-Unfreeze-FromStep2000-Seed20260426.sh"
...
cp "/mnt/task_runtime/lucidvsr/wanvideo/data/flashvsr/datasets/tar_streaming_dataset_v532_yubari_frames.py" "${RUN_DIR}/snapshot/" || true

CMD=(/mnt/conda_envs/flashvsr/bin/accelerate launch --config_file "${ACCEL_YAML}" --num_machines 2 --num_processes 16 --machine_rank "${MACHINE_RANK}" --main_process_ip "${MASTER_ADDR}" --main_process_port "${MASTER_PORT}" --deepspeed_multinode_launcher standard "${TRAIN_PY}" --config "${CONFIG_PATH}" --output_path "${RUN_DIR}/output" --wandb_name "${RUN_NAME}" --resume_stage1_checkpoint "${STAGE1_CKPT}" --lq_proj_checkpoint "${LQ_PROJ_CKPT}" --zero_init_lq_proj_in false)
