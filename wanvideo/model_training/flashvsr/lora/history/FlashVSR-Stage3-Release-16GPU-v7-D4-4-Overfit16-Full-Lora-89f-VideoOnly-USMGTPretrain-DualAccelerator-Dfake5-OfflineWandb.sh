...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_16gpu_v7_d4_4_overfit16_full_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_16gpu_v7_d4_4_overfit16_full_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb}"
...
TRAIN_PY="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/train_flashvsr_stage3_v7_d4_4_overfit_lora.py"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d44_overfit16 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d44_overfit16 \
  ...
  echo "wandb_offline_package_tmux=wandb_package_v7d44_overfit16 interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi

TAKANO_MANIFEST="/mnt/task_wrapper/user_output/artifacts/data/manifests/takano_video_train_all.txt"
OVERFIT_MANIFEST="/mnt/task_wrapper/user_output/artifacts/data/manifests/stage3_overfit16_takano_fixed_20260523.txt"
...
awk 'NF && $1 !~ /^#/ { print; n += 1; if (n == 16) exit 0 }' "${TAKANO_MANIFEST}" > "${OVERFIT_MANIFEST}"
if [ "$(wc -l < "${OVERFIT_MANIFEST}")" -ne 16 ]; then
  echo "[error] overfit manifest should contain exactly 16 lines: ${OVERFIT_MANIFEST}" >&2
  exit 1
fi
...
echo "stage3_v7_d4_4_overfit16_full=1"
...
echo "max_source_frames=90 gives a fixed 89-frame crop per source video"
