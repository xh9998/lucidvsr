...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_16gpu_v7_d4_4_flow0p1_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_16gpu_v7_d4_4_flow0p1_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d44_flow0p1_16gpu 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d44_flow0p1_16gpu \
  ...
  echo "wandb_offline_package_tmux=wandb_package_v7d44_flow0p1_16gpu interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi
...
echo "stage3_v7_d4_4_flow0p1=1 student_losses=flow0p1_mse1_lpips2_dmd1 fake_fm=1 dfake_gen_update_ratio=5"
echo "equiv_samples: 16gpu step250 ~= 40gpu step100; 16gpu step500 ~= 40gpu step200"
