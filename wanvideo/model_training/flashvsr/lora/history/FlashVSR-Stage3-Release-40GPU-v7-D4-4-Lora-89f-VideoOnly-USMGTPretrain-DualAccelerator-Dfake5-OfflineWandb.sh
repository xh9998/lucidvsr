...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_40gpu_v7_d4_4_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_40gpu_v7_d4_4_lora_89f_videoonly_usmgtpretrain_dualaccelerator_dfake5_offlinewandb}"
TEMPLATE_YAML="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/lora/history/accelerate_zero2_flashvsr_5node40gpu_nooffload.template.yaml"
...
: "${MASTER_PORT:=29500}"
: "${MACHINE_RANK:?MACHINE_RANK must be set (0..4)}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d44_40gpu_dfake5 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d44_40gpu_dfake5 \
    "cd /mnt/task_runtime/lucidvsr && RUN_DIR='${RUN_DIR}' TRAIN_PROCESS_PATTERN='train_flashvsr_stage3_v7_d4_4_lora.py' WANDB_OFFLINE_S3_URI='${WANDB_OFFLINE_S3_URI}' WANDB_PACKAGE_INTERVAL_SECONDS=3600 bash /mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/scripts/package_wandb_offline_to_s3_loop.sh"
  echo "wandb_offline_package_tmux=wandb_package_v7d44_40gpu_dfake5 interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
  if [ "${FLASHVSR_REMOTE_WANDB_SYNC:-1}" = "1" ]; then
    WANDB_SYNC_TMUX="${WANDB_SYNC_TMUX:-wandb_sync_v7d44_40gpu_dfake5}"
    WANDB_SYNC_INTERVAL_SECONDS="${WANDB_SYNC_INTERVAL_SECONDS:-900}"
    WANDB_SYNC_RUN_ID="${WANDB_RUN_ID:-}"
    tmux kill-session -t "${WANDB_SYNC_TMUX}" 2>/dev/null || true
    tmux new-session -d -s "${WANDB_SYNC_TMUX}" \
      "RUN_DIR='${RUN_DIR}' WANDB_SYNC_RUN_ID='${WANDB_SYNC_RUN_ID}' WANDB_SYNC_INTERVAL_SECONDS='${WANDB_SYNC_INTERVAL_SECONDS}' bash -lc 'while true; do date; export HTTP_PROXY=http://proxy.config.pcp.local:3128 HTTPS_PROXY=http://proxy.config.pcp.local:3128 http_proxy=http://proxy.config.pcp.local:3128 https_proxy=http://proxy.config.pcp.local:3128; source /miniforge/etc/profile.d/conda.sh; conda activate flashvsr; if [ -n \"\${WANDB_SYNC_RUN_ID}\" ]; then OFFLINE_RUN=\$(ls -dt \"\${RUN_DIR}\"/wandb/offline-run-*\"\${WANDB_SYNC_RUN_ID}\" 2>/dev/null | head -1); else OFFLINE_RUN=\$(ls -dt \"\${RUN_DIR}\"/wandb/offline-run-* 2>/dev/null | head -1); fi; if [ -n \"\${OFFLINE_RUN}\" ]; then wandb sync --include-offline \"\${OFFLINE_RUN}\"; else echo \"waiting_for_wandb_offline_run run_dir=\${RUN_DIR}\"; fi; sleep \"\${WANDB_SYNC_INTERVAL_SECONDS}\"; done'"
    echo "wandb_remote_sync_tmux=${WANDB_SYNC_TMUX} interval_seconds=${WANDB_SYNC_INTERVAL_SECONDS} run_id=${WANDB_SYNC_RUN_ID:-latest}"
  fi
fi
...
echo "stage3_v7_d4_4=dual_accelerator_deepspeed fake_fm_current_zpred_detach=1 dmd=1 shared_dmd_noisy_point usmgt_stage1_teacher_step3000 teacher_lq_trim_tail_to_match spike_skip5 dfake_gen_update_ratio=5 validation=c6_stable"
echo "distributed_shape=5node40gpu"

ACCEL_YAML="${RUN_DIR}/accelerate_5node40gpu.yaml"
