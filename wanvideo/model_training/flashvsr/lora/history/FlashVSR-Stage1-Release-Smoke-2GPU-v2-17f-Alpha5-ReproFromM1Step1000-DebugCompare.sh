...
RUN_NAME="${OUTPUT_TAG:-train_stage1_release_smoke_2gpu_v2_17f_alpha5_repro_debug_compare}_${RUN_TS}"
...
cp "$REPO_DIR/wanvideo/model_training/flashvsr/train_flashvsr_stage1_v2_debug_compare.py" "$RUN_DIR/snapshot/" || true
...
printf '%q ' env CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" /mnt/conda_envs/flashvsr/bin/accelerate launch \
  ...
  "$REPO_DIR/wanvideo/model_training/flashvsr/train_flashvsr_stage1_v2_debug_compare.py" \
  --config "$CONFIG_PATH" \
