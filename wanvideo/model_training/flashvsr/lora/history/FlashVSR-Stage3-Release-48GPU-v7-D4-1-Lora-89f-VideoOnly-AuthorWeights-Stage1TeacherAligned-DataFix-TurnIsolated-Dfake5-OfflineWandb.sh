...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d4_1_lora_89f_videoonly_authorweights_stage1teacher_aligned_datafix_turnisolated_dfake5_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d4_1_lora_89f_videoonly_authorweights_stage1teacher_aligned_datafix_turnisolated_dfake5_offlinewandb}"
...
TRAIN_PY="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/train_flashvsr_stage3_v7_d4_1_lora.py"
...
STAGE3_REAL_S3="${STAGE3_REAL_S3:-s3://lxh/artifacts/exp/${STAGE1_EXP}/output/step-3000.safetensors}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d41_turnisolated_dfake5 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d41_turnisolated_dfake5 \
    "cd /mnt/task_runtime/lucidvsr && RUN_DIR='${RUN_DIR}' TRAIN_PROCESS_PATTERN='train_flashvsr_stage3_v7_d4_1_lora.py' WANDB_OFFLINE_S3_URI='${WANDB_OFFLINE_S3_URI}' WANDB_PACKAGE_INTERVAL_SECONDS=3600 bash /mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/scripts/package_wandb_offline_to_s3_loop.sh"
  echo "wandb_offline_package_tmux=wandb_package_v7d41_turnisolated_dfake5 interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi
...
echo "stage3_v7_d4_1=author_weights fake_fm=1 dmd=1 shared_dmd_noisy_point stage1_teacher_nonstreaming_aligned teacher_lq_trim_front_to_match spike_skip5 dmd2_alternating dfake_gen_update_ratio=5 validation=c6_stable"
