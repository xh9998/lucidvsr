...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d4_2_lora_89f_videoonly_usmgtpretrain_turnisolated_dfake5_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d4_2_lora_89f_videoonly_usmgtpretrain_turnisolated_dfake5_offlinewandb}"
...
STAGE3_REAL_S3="${STAGE3_REAL_S3:-s3://lxh/tmp/usmgt_stage1_takano20250205_step3000_20260517/step-3000.safetensors}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d42_usmgt_dfake5 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d42_usmgt_dfake5 \
  ...
  echo "wandb_offline_package_tmux=wandb_package_v7d42_usmgt_dfake5 interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi
...
echo "stage3_v7_d4_2=usmgt_stage1_step3000 fake_fm=1 dmd=1 shared_dmd_noisy_point stage1_teacher_nonstreaming_aligned teacher_lq_trim_front_to_match spike_skip5 turn_isolated dfake_fake_updates_per_generator=5 validation=c6_stable"
