...
CONFIG_PATH=${CONFIG_PATH:-$REPO_ROOT/wanvideo/model_training/flashvsr/configs/history/stage1_release_smoke_2gpu_v4_lora_joint_17f_mix0404020_aliyundegra.yaml}
ACCELERATE_CONFIG=${ACCELERATE_CONFIG:-$REPO_ROOT/wanvideo/model_training/flashvsr/lora/accelerate_zero2_flashvsr_2gpu.yaml}
OUTPUT_TAG=${OUTPUT_TAG:-train_stage1_release_smoke_2gpu_v4_lora_joint_17f_mix0404020_aliyundegra}
...
source /mnt/task_runtime/bolt_lxh/use_active_python.sh
CONDUCTOR_VERBOSITY=1 CONDUCTOR_METRICS_INTERVAL=3600000 CONDUCTOR_CACHE_MAX_BYTES=214748364800 CONDUCTOR_CACHE_TARGET_PCT=0.80 CONDUCTOR_CACHE_TTL_SECONDS=21600 \\
...
cd "$REPO_ROOT"
nohup env CONDUCTOR_VERBOSITY=1 CONDUCTOR_METRICS_INTERVAL=3600000 CONDUCTOR_CACHE_MAX_BYTES=214748364800 CONDUCTOR_CACHE_TARGET_PCT=0.80 CONDUCTOR_CACHE_TTL_SECONDS=21600 \
  FLASHVSR_IO_MAX_PARALLEL=4 FLASHVSR_IO_NODE_LIMIT_DIR=/tmp/flashvsr_io_limiter \
  PYTHONDONTWRITEBYTECODE=1 \
  accelerate launch --config_file "$ACCELERATE_CONFIG" \
  wanvideo/model_training/flashvsr/train_flashvsr_stage1_v4_lora.py \
