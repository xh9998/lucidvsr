...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d3_1_lora_89f_videoonly_authorweights_stage1teacher_aligned_clean_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d3_1_lora_89f_videoonly_authorweights_stage1teacher_aligned_clean_offlinewandb}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d31_stage1teacher_clean 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d31_stage1teacher_clean \
  ...
  echo "wandb_offline_package_tmux=wandb_package_v7d31_stage1teacher_clean interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi
...
echo "stage3_v7_d3_1=author_weights fake_fm=1 dmd=1 shared_dmd_noisy_point stage1_teacher_nonstreaming_aligned teacher_lq_trim_front_to_match spike_skip5 validation=c6_stable"
