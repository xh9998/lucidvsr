...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d3_1_highutil_lora_89f_videoonly_authorweights_stage1teacher_aligned_clean_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d3_1_highutil_lora_89f_videoonly_authorweights_stage1teacher_aligned_clean_offlinewandb}"
...
TRAIN_PY="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/train_flashvsr_stage3_v7_d3_1_lora.py"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  ...
  tmux kill-session -t wandb_package_v7d31_highutil_stage1teacher_clean 2>/dev/null || true
  tmux new-session -d -s wandb_package_v7d31_highutil_stage1teacher_clean \
    "cd /mnt/task_runtime/lucidvsr && RUN_DIR='${RUN_DIR}' TRAIN_PROCESS_PATTERN='train_flashvsr_stage3_v7_d3_1_lora.py' WANDB_OFFLINE_S3_URI='${WANDB_OFFLINE_S3_URI}' WANDB_PACKAGE_INTERVAL_SECONDS=3600 bash /mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/scripts/package_wandb_offline_to_s3_loop.sh"
  echo "wandb_offline_package_tmux=wandb_package_v7d31_highutil_stage1teacher_clean interval_seconds=3600 run_dir=${RUN_DIR} s3_uri=${WANDB_OFFLINE_S3_URI}"
fi
...
echo "stage3_v7_d3_1_highutil=author_weights fake_fm=1 dmd=1 shared_dmd_noisy_point stage1_teacher_nonstreaming_aligned teacher_lq_trim_front_to_match spike_skip5 validation=off decoder_cpu_offload=false"
