...
# Do not inherit a stale outer-shell PYTHON_BIN such as the default b200 env.
# Stage3 depends on the flashvsr env for wandb, flash-attn, LPIPS, and DiffSynth.
...
export WANDB_GRAPHQL_RETRY_MAX="${WANDB_GRAPHQL_RETRY_MAX:-3}"
export WANDB_GRAPHQL_TIMEOUT_SECONDS="${WANDB_GRAPHQL_TIMEOUT_SECONDS:-60}"
export WANDB_SERVICE_WAIT="${WANDB_SERVICE_WAIT:-120}"

CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d_lora_89f_videoonly_authorweights_trainablefake_641data.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d_lora_89f_videoonly_authorweights_trainablefake_641data}"
...
TRAIN_PY="/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/train_flashvsr_stage3_v7_c_lora.py"
...
echo "stage3_v7_d=author_weights fake_fm=1 dmd=1 dmd_spike=skip@5 validation=streaming_kvcache_one_step"
