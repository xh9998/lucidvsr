...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_d42_fixedlqgt_4gpu_dmdonly_singlerunner_dfake5.yaml}"
GPU_IDS="${GPU_IDS:-0,1,2,3}"
MASTER_PORT="${MASTER_PORT:-29573}"
OUTPUT_TAG="${OUTPUT_TAG:-stage3_D42_fixedlqgt_4gpu_dmdonly_singlerunner_dfake5}"
START_OCCUPY_ON_EXIT="${START_OCCUPY_ON_EXIT:-1}"
START_EMPTY_GPU_GUARD="${START_EMPTY_GPU_GUARD:-1}"

export CUDA_VISIBLE_DEVICES="${GPU_IDS}"
SAFE_ID="${OUTPUT_TAG//-/_}"
SAFE_ID="${SAFE_ID//./_}"
...
finish() {
  ...
  if [ "${START_OCCUPY_ON_EXIT}" = "1" ]; then
    echo "[guard] ${OUTPUT_TAG} exited rc=${rc}; starting occupy on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
...
if [ "${START_EMPTY_GPU_GUARD}" = "1" ]; then
  GPU_EMPTY_GUARD_SESSION="gpu_empty_guard_${SAFE_ID}"
  ...
  tmux new-session -d -s "${GPU_EMPTY_GUARD_SESSION}" \
    "cd /mnt/task_runtime/lucidvsr && GPU_IDS='${GPU_IDS}' GPU_EMPTY_GUARD_LOG_PREFIX='[gpu-empty-guard ${OUTPUT_TAG}]' bash /mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/scripts/gpu_empty_guard_start_occupy.sh 2>&1 | tee -a '${GPU_EMPTY_GUARD_LOG}'"
...
echo "experiment=${OUTPUT_TAG}"
echo "purpose=D4.2 single-runner DMD-only control for fake-branch yellow/gray drift"
