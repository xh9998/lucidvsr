...
CONFIG_PATH=${CONFIG_PATH:-$REPO_ROOT/wanvideo/model_training/flashvsr/configs/history/stage1_release_smoke_2gpu_v5_1_lora_17f_tarv5_yubari_highresroot_1to1.yaml}
ACCELERATE_CONFIG=${ACCELERATE_CONFIG:-$REPO_ROOT/wanvideo/model_training/flashvsr/lora/accelerate_zero2_flashvsr_2gpu.yaml}
OUTPUT_TAG=${OUTPUT_TAG:-train_stage1_release_smoke_2gpu_v5_1_lora_17f_tarv5_yubari_highresroot_1to1}
...
nohup accelerate launch --config_file "$ACCELERATE_CONFIG" "$REPO_ROOT/wanvideo/model_training/flashvsr/train_flashvsr_stage1_v5_1_lora.py" --config "$CONFIG_PATH" --output_path "$RUN_DIR/output" --wandb_name "$RUN_NAME" > "$RUN_DIR/run.log" 2>&1 < /dev/null &
