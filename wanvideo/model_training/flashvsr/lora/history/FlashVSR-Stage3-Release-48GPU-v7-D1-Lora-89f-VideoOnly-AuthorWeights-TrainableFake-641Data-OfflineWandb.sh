...
CONFIG_PATH="${CONFIG_PATH:-/mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/configs/history/stage3_release_48gpu_v7_d1_lora_89f_videoonly_authorweights_trainablefake_641data_offlinewandb.yaml}"
OUTPUT_TAG="${OUTPUT_TAG:-train_stage3_release_48gpu_v7_d1_lora_89f_videoonly_authorweights_trainablefake_641data}"
...
if [ "${MACHINE_RANK}" = "0" ]; then
  tmux kill-session -t wandb_sync_v7d1 2>/dev/null || true
  tmux new-session -d -s wandb_sync_v7d1 \
    "cd /mnt/task_runtime/lucidvsr && RUN_DIR='${RUN_DIR}' TRAIN_PROCESS_PATTERN='train_flashvsr_stage3_v7_d1_lora.py' WANDB_SYNC_INTERVAL_SECONDS=3600 bash /mnt/task_runtime/lucidvsr/wanvideo/model_training/flashvsr/scripts/sync_wandb_offline_loop.sh"
  echo "wandb_offline_sync_tmux=wandb_sync_v7d1 interval_seconds=3600 run_dir=${RUN_DIR}"
fi
...
echo "stage3_v7_d1=author_weights fake_fm=1 dmd=1 spike_skip5 validation=2_streaming_kvcache_one_step"
