...
class WanVideoUnit_InputVideoEmbedderStage2(PipelineUnit):
    ...
    def process(self, pipe, input_video, noise, tiled, tile_size, tile_stride, framewise_decoding):
        ...
        if input_video.shape[2] < target_frames:
            raise ValueError(
                f"Stage2 v6.4 needs at least {target_frames} GT frames for "
                f"{noise.shape[2]} latent targets, got video={tuple(input_video.shape)}"
            )
        # v6.4 keeps the first GT latent. For 89-frame streaming input the LQ
        # branch still produces 22 conditions, but the supervised target is the
        # first 85 frames, whose WAN VAE encoding is also 22 latents.
        input_video = input_video[:, :, :target_frames].contiguous()
        ...
        if noise.shape != input_latents.shape:
            raise ValueError(
                f"Stage2 v6.4 noise/target mismatch: noise={tuple(noise.shape)} "
                f"target={tuple(input_latents.shape)} target_frames={target_frames}"
            )
...
def _sample_pixel_recon_window(latent_t: int, recon_num: int, device: torch.device) -> Tuple[int, int]:
    recon_num = min(max(int(recon_num), 1), int(latent_t))
    max_start = int(latent_t) - recon_num
    if max_start <= 0:
        return 0, recon_num
    forced_start = os.environ.get("FLASHVSR_STAGE7E_FORCE_RECON_START")
...
def _unscale_vae_latents(vae, z: torch.Tensor) -> torch.Tensor:
    scale = vae.scale
...
def _decode_selected_window_full_frame(pipe, z_pred: torch.Tensor, start: int, end: int) -> torch.Tensor:
    vae = pipe.vae
    model = vae.model
    model.clear_cache()
    if start > 0:
        with torch.no_grad():
            z_prefix = _unscale_vae_latents(vae, z_pred[:, :, :start].detach())
    ...
    z_selected = _unscale_vae_latents(vae, z_pred[:, :, start:end])
...
def _decode_selected_with_checkpoint(pipe, z_pred: torch.Tensor, start: int, end: int, *, cpu_offload: bool) -> torch.Tensor:
    def _decode_selected(latents: torch.Tensor) -> torch.Tensor:
        return _decode_selected_window_full_frame(pipe, latents, start, end)
...
class _LazyLPIPS(torch.nn.Module):
    def __init__(self, net: str = "vgg"):
        ...
        try:
            ...
            raise RuntimeError("v7E requires lpips when stage2_lpips_weight > 0.") from error
...
def _get_lpips(pipe, net: str):
    if hasattr(pipe, "_modules") and "_stage7e_lpips" in pipe._modules:
        del pipe._modules["_stage7e_lpips"]
    cache = pipe.__dict__.get("_stage7e_lpips_cache")
    cache_net = pipe.__dict__.get("_stage7e_lpips_cache_net")
    if cache is None or cache_net != net:
        cache = _LazyLPIPS(net=net).to(device=pipe.device)
        cache.eval()
        object.__setattr__(pipe, "_stage7e_lpips_cache", cache)
        object.__setattr__(pipe, "_stage7e_lpips_cache_net", net)
    return cache


def _lpips_video_loss(
...
def FlowMatchPixelLossStage2V7E(
    pipe: BasePipeline,
    *,
    stage2_flow_weight: float,
    stage2_mse_weight: float,
    stage2_lpips_weight: float,
    stage2_lpips_net: str,
    stage2_recon_num_latents: int,
    stage2_first_frame_pixel_weight: float,
    stage2_first_frame_lpips_weight: float,
    stage2_decoder_cpu_offload: bool,
    **inputs,
):
    ...
    need_reconstruction = float(stage2_mse_weight) != 0.0 or float(stage2_lpips_weight) != 0.0
    if need_reconstruction:
        ...
        recon_start, recon_end = _sample_pixel_recon_window(latent_t, int(stage2_recon_num_latents), z_pred.device)
        frame_start, frame_end = _latent_window_to_frame_range(recon_start, recon_end)
        pipe.load_models_to_device(["vae"])
        x_pred = _decode_selected_with_checkpoint(
            pipe,
            z_pred,
            recon_start,
            recon_end,
            cpu_offload=bool(stage2_decoder_cpu_offload),
        ).to(device=pipe.device).contiguous()
        ...
        if x_gt.shape != x_pred.shape:
            raise ValueError(f"v7E decoded/GT mismatch: pred={tuple(x_pred.shape)} gt={tuple(x_gt.shape)}")
        first_pixel = float(stage2_first_frame_pixel_weight) if frame_start == 0 else 1.0
        first_lpips = float(stage2_first_frame_lpips_weight) if frame_start == 0 else 1.0
        mse_loss = _weighted_video_mse(x_pred, x_gt, first_frame_weight=first_pixel)
        if float(stage2_lpips_weight) != 0.0:
            lpips_loss = _lpips_video_loss(
                _get_lpips(pipe, stage2_lpips_net),
                x_pred,
                x_gt,
                first_frame_weight=first_lpips,
                cpu_offload=bool(stage2_decoder_cpu_offload),
            )
        else:
            lpips_loss = torch.zeros((), device=pipe.device, dtype=torch.float32)
    else:
        recon_start = recon_end = frame_start = frame_end = 0
        first_pixel = first_lpips = 1.0
        mse_loss = torch.zeros((), device=pipe.device, dtype=torch.float32)
        lpips_loss = torch.zeros((), device=pipe.device, dtype=torch.float32)

    total = float(stage2_flow_weight) * flow_loss + float(stage2_mse_weight) * mse_loss + float(stage2_lpips_weight) * lpips_loss
    pipe._stage7e_last_losses = {
        "loss": float(total.detach().cpu()),
        "loss_flow": float(flow_loss.detach().cpu()),
        "loss_mse": float(mse_loss.detach().cpu()),
        "loss_lpips": float(lpips_loss.detach().cpu()),
        "recon_latent_start": int(recon_start),
        "recon_latent_end": int(recon_end),
        "decoded_frame_start": int(frame_start),
        "decoded_frame_end": int(frame_end),
        "first_frame_pixel_weight": float(first_pixel),
        "first_frame_lpips_weight": float(first_lpips),
    }
    if os.environ.get("FLASHVSR_STAGE7E_DEBUG_LOSS", "1") == "1" and os.environ.get("RANK", "0") == "0":
        print(
            "[stage2_v7e_loss] "
            f"loss={pipe._stage7e_last_losses['loss']:.6f} "
            f"flow={pipe._stage7e_last_losses['loss_flow']:.6f} "
            f"mse={pipe._stage7e_last_losses['loss_mse']:.6f} "
            f"lpips={pipe._stage7e_last_losses['loss_lpips']:.6f} "
            f"latent=[{recon_start},{recon_end}) frame=[{frame_start},{frame_end})",
            flush=True,
        )
    return total
...
class FlashVSRStage2Pipeline(WanVideoPipeline):
    ...
    def infer_from_lq_streaming(
        ...
        v6.4 trains the same 22-latent stream as inference, but supervises it
        as the WAN encoding of the first 85 target frames rather than
        89-frame encoding with the first latent dropped.
        """
...
def flashvsr_stage2_model_fn(
    ...
    **kwargs,
    ...
    if lq_latents is not None:
        ...
        if lq_latent_alignment in ("trim_front_to_match", "trim_tail_to_match"):
            original_tokens = int(lq_latents[0].shape[1]) if lq_latents else None
            if lq_latent_alignment == "trim_front_to_match":
                lq_latents = v5._align_lq_latents_to_dit_tokens(
                    lq_latents,
                    expected_tokens=expected_tokens,
                    tokens_per_frame=h * w,
                    raw_segment_lengths=None,
                )
            else:
                aligned_lq_latents = []
                for layer_latents in lq_latents:
                    current_tokens = int(layer_latents.shape[1])
                    if current_tokens < expected_tokens:
                        raise ValueError(
                            f"Cannot trim tail lq_latents to x tokens: x={expected_tokens}, "
                            f"lq={current_tokens}, tokens_per_frame={h * w}"
                        )
                    trim_tokens = current_tokens - int(expected_tokens)
                    if trim_tokens % (h * w) != 0:
                        raise ValueError(
                            f"Cannot trim tail lq_latents to x tokens: x={expected_tokens}, "
                            f"lq={current_tokens}, tokens_per_frame={h * w}"
                        )
                    aligned_lq_latents.append(layer_latents[:, :expected_tokens, :].contiguous())
                lq_latents = aligned_lq_latents
            if os.environ.get("FLASHVSR_STAGE3_TEACHER_ALIGN_DEBUG", "0") == "1":
                trim_frames = None
                if original_tokens is not None:
                    trim_tokens = original_tokens - int(expected_tokens)
                    if trim_tokens >= 0 and (h * w) > 0 and trim_tokens % (h * w) == 0:
                        trim_frames = int(trim_tokens // (h * w))
                semantic_map = ""
                if trim_frames is not None:
                    if lq_latent_alignment == "trim_front_to_match":
                        drop_positions = f"[0,{trim_frames})"
                        keep_positions = f"[{trim_frames},{trim_frames + int(f)})"
                        note = "teacher_position0_is_nonstreaming_aligned_warmup"
                    else:
                        teacher_positions = original_tokens // (h * w)
                        drop_positions = f"[{int(f)},{teacher_positions})"
                        keep_positions = f"[0,{int(f)})"
                        note = "teacher_front_positions_match_stage2_v64_target"
                    semantic_map = (
                        f" teacher_tokens_before={original_tokens} "
                        f"teacher_positions_before={original_tokens // (h * w)} "
                        f"drop_teacher_positions={drop_positions} "
                        f"keep_teacher_positions={keep_positions} "
                        f"student_positions=[0,{int(f)}) "
                        f"note={note}"
                    )
                print(
                    "[stage3_teacher_align] "
                    f"mode={lq_latent_alignment} grid={(int(f), int(h), int(w))} "
                    f"expected_tokens={expected_tokens} "
                    f"aligned_lq_shape={tuple(lq_latents[0].shape) if lq_latents else None}"
                    f"{semantic_map}",
                    flush=True,
                )
        for layer_idx, layer_latents in enumerate(lq_latents):
            if layer_latents.shape[1] != expected_tokens:
                raise ValueError(
                    ...
                    f"alignment={lq_latent_alignment}"
                )
...
class FlashVSRStage2TrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        stage2_topk_ratio: float = 2.0,
        stage2_local_num: int = -1,
        stage2_flow_weight: float = 1.0,
        stage2_mse_weight: float = 1.0,
        stage2_lpips_weight: float = 2.0,
        stage2_lpips_net: str = "vgg",
        stage2_recon_num_latents: int = 2,
        stage2_first_frame_pixel_weight: float = 4.0,
        stage2_first_frame_lpips_weight: float = 4.0,
        stage2_decoder_cpu_offload: bool = True,
        lq_proj_temporal_mode: str = "streaming",
        fp8_models=None,
        offload_models=None,
        device="cpu",
        ...
        self.stage2_flow_weight = float(stage2_flow_weight)
        self.stage2_mse_weight = float(stage2_mse_weight)
        self.stage2_lpips_weight = float(stage2_lpips_weight)
        self.stage2_lpips_net = str(stage2_lpips_net)
        self.stage2_recon_num_latents = int(stage2_recon_num_latents)
        self.stage2_first_frame_pixel_weight = float(stage2_first_frame_pixel_weight)
        self.stage2_first_frame_lpips_weight = float(stage2_first_frame_lpips_weight)
        self.stage2_decoder_cpu_offload = bool(stage2_decoder_cpu_offload)
    ...
    def forward(self, data, inputs=None):
        ...
        return FlowMatchPixelLossStage2V7E(
            self.pipe,
            stage2_flow_weight=self.stage2_flow_weight,
            stage2_mse_weight=self.stage2_mse_weight,
            stage2_lpips_weight=self.stage2_lpips_weight,
            stage2_lpips_net=self.stage2_lpips_net,
            stage2_recon_num_latents=self.stage2_recon_num_latents,
            stage2_first_frame_pixel_weight=self.stage2_first_frame_pixel_weight,
            stage2_first_frame_lpips_weight=self.stage2_first_frame_lpips_weight,
            stage2_decoder_cpu_offload=self.stage2_decoder_cpu_offload,
            **merged_inputs[0],
            **merged_inputs[1],
        )
...
def _stage2_parser():
    ...
    parser.add_argument("--resume_stage2_checkpoint", type=str, default=None, help="Stage2 6.4.1 checkpoint used to initialize v7E.")
    parser.add_argument("--stage2_flow_weight", type=float, default=1.0)
    parser.add_argument("--stage2_mse_weight", type=float, default=1.0)
    parser.add_argument("--stage2_lpips_weight", type=float, default=2.0)
    parser.add_argument("--stage2_lpips_net", type=str, default="vgg", choices=("alex", "vgg", "squeeze"))
    parser.add_argument("--stage2_recon_num_latents", type=int, default=2)
    parser.add_argument("--stage2_first_frame_pixel_weight", type=float, default=4.0)
    parser.add_argument("--stage2_first_frame_lpips_weight", type=float, default=4.0)
    parser.add_argument("--stage2_decoder_cpu_offload", action="store_true", default=True)
    parser.add_argument("--no_stage2_decoder_cpu_offload", dest="stage2_decoder_cpu_offload", action="store_false")
    return parser
...
def main(argv=None):
    ...
    model = FlashVSRStage2TrainingModule(
        ...
        stage2_flow_weight=args.stage2_flow_weight,
        stage2_mse_weight=args.stage2_mse_weight,
        stage2_lpips_weight=args.stage2_lpips_weight,
        stage2_lpips_net=args.stage2_lpips_net,
        stage2_recon_num_latents=args.stage2_recon_num_latents,
        stage2_first_frame_pixel_weight=args.stage2_first_frame_pixel_weight,
        stage2_first_frame_lpips_weight=args.stage2_first_frame_lpips_weight,
        stage2_decoder_cpu_offload=args.stage2_decoder_cpu_offload,
    ...
    if accelerator.is_local_main_process:
        ...
        print(
            "Stage2 v7E loss weights: "
            f"flow={args.stage2_flow_weight} mse={args.stage2_mse_weight} "
            f"lpips={args.stage2_lpips_weight} recon_latents={args.stage2_recon_num_latents} "
            f"decoder_cpu_offload={args.stage2_decoder_cpu_offload}",
            flush=True,
        )
