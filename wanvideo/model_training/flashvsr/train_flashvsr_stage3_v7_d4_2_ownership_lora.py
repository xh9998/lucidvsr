...
def _stage3_ownership_debug_enabled() -> bool:
    return os.environ.get("FLASHVSR_STAGE3_GRAD_OWNERSHIP_DEBUG", "0").strip().lower() in {"1", "true", "yes"}
...
def _maybe_log_stage3_grad_ownership(
    ...
    *,
    ...
    if not (_stage3_ownership_debug_enabled() and accelerator.is_main_process):
        return
    dmd_value = "None" if dmd_student_loss is None else f"{float(dmd_student_loss.detach().float().mean().cpu().item()):.8f}"
    print(
        "[stage3_grad_ownership_d42] "
...
def launch_stage3c_dual_optimizer_task(
    ...
    if accelerator.is_main_process:
        ...
        if fixed_lqgt_root:
            print(
                "[stage3_ownership_d42] Using pre-generated fixed LQ/GT tensors "
                f"root={fixed_lqgt_root}; online degradation output will be ignored.",
                flush=True,
            )
        print(
            "[stage3c_runner] D4.2 single-runner separated optimizer "
        ...
        while True:
            ...
            if fixed_lqgt_root:
                if fixed_lqgt_batch is None:
                    ...
                    if accelerator.is_main_process:
                        print("[stage3_ownership_d42] Loaded fixed LQ/GT batch for reuse.", flush=True)
            ...
            with accelerator.accumulate(model):
                ownership_before = {
                    ...
                    "fake": _stage3_param_checksum(fake_model, trainable_only=True),
                    "real": _stage3_param_checksum(real_model, trainable_only=False),
                } if _stage3_ownership_debug_enabled() else None
                optimizer.zero_grad()
