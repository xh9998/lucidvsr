...
def _stage3_fixed_timestep_id(min_timestep_boundary: int, max_timestep_boundary: int) -> Optional[torch.Tensor]:
    raw = os.environ.get("FLASHVSR_STAGE3_FIXED_DMD_TIMESTEP_ID", "").strip()
    if not raw:
        return None
    value = int(raw)
    value = max(int(min_timestep_boundary), min(int(max_timestep_boundary) - 1, value))
    return torch.tensor([value], dtype=torch.long)


def _stage3_noise_like_for_dmd(latents: torch.Tensor) -> torch.Tensor:
    raw = os.environ.get("FLASHVSR_STAGE3_FIXED_DMD_NOISE_SEED", "").strip()
    if not raw:
        return torch.randn_like(latents)
    rank = int(os.environ.get("RANK", os.environ.get("LOCAL_RANK", "0")))
    generator = torch.Generator(device=latents.device)
    generator.manual_seed(int(raw) + rank)
...
def _stage3_load_fixed_lqgt_batch(root: str, process_index: int) -> Dict[str, Any]:
    ...
    sample_seed = int(payload.get("sample_seed", process_index))
    sample_id = str(payload.get("sample_id", sample_path.stem))
    if video.ndim != 4 or lq_video.ndim != 4:
        raise ValueError(f"Expected fixed tensors [T,C,H,W], got video={tuple(video.shape)} lq={tuple(lq_video.shape)}")
    return {
        ...
        "sample_id": [sample_id],
        "source_type": ["fixed_lqgt_ownership"],
    }
    ...
    *,
    ...
    try:
        ...
        if dmd_point is None:
            ...
            timestep_id = _stage3_fixed_timestep_id(min_timestep_boundary, max_timestep_boundary)
            if timestep_id is None:
                timestep_id = torch.randint(min_timestep_boundary, max_timestep_boundary, (1,))
            timestep = pipe.scheduler.timesteps[timestep_id].to(dtype=pipe.torch_dtype, device=pipe.device)
            noise = _stage3_noise_like_for_dmd(clean_latents)
...
def _stage3c_fake_fm_loss(
    ...
    *,
    ...
    if timestep_id is None:
        timestep_id = torch.randint(min_timestep_boundary, max_timestep_boundary, (1,))
    timestep = pipe.scheduler.timesteps[timestep_id].to(dtype=pipe.torch_dtype, device=pipe.device)
    noise = _stage3_noise_like_for_dmd(fake_clean_latents)
...
def launch_stage3c_dual_optimizer_task(
    ...
    if accelerator.is_main_process:
        ...
        if fixed_lqgt_root:
            print(
                "[stage3_ownership_d44] Using pre-generated fixed LQ/GT tensors "
    ...
    for epoch_id in range(resumed_epoch_id, num_epochs):
        ...
        while True:
            ...
            timing_parts = {}
            if fixed_lqgt_root:
                if fixed_lqgt_batch is None:
                    fixed_lqgt_batch = _stage3_load_fixed_lqgt_batch(fixed_lqgt_root, accelerator.process_index)
                    if accelerator.is_main_process:
                        print("[stage3_ownership_d44] Loaded fixed LQ/GT batch for reuse.", flush=True)
                data = fixed_lqgt_batch
            else:
                try:
