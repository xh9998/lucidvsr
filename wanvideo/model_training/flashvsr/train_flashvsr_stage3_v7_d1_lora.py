...
def _build_stage3_decode_window(
    ...
    v7-D1's production path below does not backprop through this whole prefix.
...
class _LazyLPIPS(torch.nn.Module):
    def __init__(self, net: str = "vgg"):
        ...
        try:
            ...
            raise RuntimeError(
                "Stage3 v7-D1 requires the `lpips` package when stage3_lpips_weight > 0. "
...
def Stage3BOneStepReconLoss(pipe, *, stage3_recon_num_latents: int, stage3_flow_weight: float,
    ...
    if float(stage3_fake_fm_weight) != 0.0:
        raise RuntimeError(
            "Stage3 v7-D1 fake-FM is intentionally guarded off in this first "
            ...
            "only owns one optimizer. Keep stage3_fake_fm_weight=0 for the v7-D1 "
    ...
    if bool(stage3_compute_z_pred) or need_reconstruction:
        # One-step prediction to sigma=0. This is the v7-D1 student output z_pred.
        z_pred = pipe.scheduler.step(noise_pred, timestep, inputs["latents"], to_final=True)
    else:
        z_pred = None
    # v7-D1 reads this immediately after the student forward to build DMD probes.
    ...
    if need_reconstruction:
        ...
        if x_gt.shape != x_pred.shape:
            raise ValueError(f"Stage3 v7-D1 decoded/GT mismatch: pred={tuple(x_pred.shape)} gt={tuple(x_gt.shape)}")
...
class FlashVSRStage3BValidationCallback(v6.FlashVSRStage2ValidationCallback):
    def __init__(self, *args, tiled: bool = False, **kwargs):
        super().__init__(*args, **kwargs)
            ...
            for sample_index, sample in enumerate(self.validation_samples):
                ...
                with torch.inference_mode():
                    sr_frames = pipe.infer_from_lq_streaming(
                        ...
                        topk_ratio=getattr(pipe, "stage2_topk_ratio", 2.0),
                        kv_ratio=3.0,
                    )
                    ...
                    json.dump(
                        {
                            ...
                            "validation_mode": "stage3_v7_d1_streaming_kvcache_one_step",
...
def collect_stage3_validation_samples(dataset, num_samples: int, num_workers: int = 0) -> List[Dict[str, Any]]:
    """Collect fixed validation samples with progress and optional CPU workers.

    Stage3 validation samples are full 89-frame degraded clips. Collecting them
    through the iterable dataset in the main process can look like a startup hang,
    especially with CPU-only Aliyun degradation. Keep the semantics identical to
    the training dataset, but allow DataLoader workers to do the expensive decode
    and degradation work.
    """
    if num_samples <= 0:
        return []
    if num_workers <= 0:
        samples = v5.collect_fixed_validation_samples(dataset, num_samples)
        print(f"[stage3_validation_collect] collected={len(samples)}/{num_samples} workers=0", flush=True)
        return samples

    def _cache_sample(sample: Dict[str, Any]) -> Dict[str, Any]:
        cached: Dict[str, Any] = {}
    ...
    loader_kwargs = {
        ...
        "pin_memory": False,
        "worker_init_fn": _init_data_worker_no_cuda,
    }
    ...
    for sample in loader:
        samples.append(_cache_sample(sample))
        print(f"[stage3_validation_collect] collected={len(samples)}/{num_samples} workers={num_workers}", flush=True)
        if len(samples) >= num_samples:
            break
    return samples


def extend_stage3_validation_samples_from_batch(
    validation_samples: List[Dict[str, Any]],
    data: Dict[str, Any],
    *,
    num_samples: int,
) -> None:
    """Cache rank0 training tensors as validation samples.

    This avoids pre-training validation sampling through the remote iterable
    dataset. With per-rank batch size 1 it will collect one sample at a time,
    then reuse the exact tensors that entered training after degradation.
    """
    if num_samples <= 0 or len(validation_samples) >= num_samples:
        return
    video = data.get("video")
    lq_video = data.get("lq_video")
    if not (torch.is_tensor(video) and torch.is_tensor(lq_video)):
    ...
    if video.ndim != 5 or lq_video.ndim != 5:
        return
    batch = min(int(video.shape[0]), int(lq_video.shape[0]))
    sample_seed = data.get("sample_seed")
    for batch_index in range(batch):
        ...
        if torch.is_tensor(sample_seed):
            if sample_seed.ndim == 0:
                seed_value = sample_seed.detach().cpu().clone()
            elif batch_index < int(sample_seed.shape[0]):
                seed_value = sample_seed[batch_index].detach().cpu().clone()
            else:
                seed_value = torch.tensor(len(validation_samples), dtype=torch.long)
        else:
            seed_value = torch.tensor(len(validation_samples), dtype=torch.long)
        validation_samples.append(
            {
                "video": video[batch_index].detach().cpu().clone(),
                "lq_video": lq_video[batch_index].detach().cpu().clone(),
                "sample_seed": seed_value,
                "validation_source": "rank0_training_batch",
            }
        )
        print(
            f"[stage3_validation_from_batch] cached={len(validation_samples)}/{num_samples} "
            f"video_shape={tuple(video[batch_index].shape)}",
            flush=True,
        )
...
class Stage3CFakeScalarModel(torch.nn.Module):
    ...
    This is intentionally not the final G_fake model. It lets v7-D1 prove the
...
def save_stage3c_extra_state(accelerator, output_path: str, step: int, fake_model, fake_optimizer, fake_scheduler, args):
    ...
    if fake_is_full_model:
        ...
        fake_state_note = "v7-D1 trainable G_fake state: trainable LoRA/lq_proj parameters only."
    else:
        fake_state = fake_model.state_dict()
        fake_state_note = "v7-D1 placeholder fake scalar state."
    payload = {
        "step": int(step),
...
def _init_stage3_wandb(args, accelerator=None):
    """Initialize W&B once on the global main process."""
    rank_env = os.environ.get("RANK")
    if rank_env is not None:
        is_global_rank0 = int(rank_env) == 0
    elif accelerator is not None:
        is_global_rank0 = accelerator.is_main_process
    else:
        is_global_rank0 = True
    if not getattr(args, "use_wandb", False) or not is_global_rank0:
        return None
    try:
        import wandb
...
def _stage3c_probe_predict_x0(probe_model, data, clean_latents: torch.Tensor, *, dataset_load_from_cache: bool, probe_name: str = "probe"):
    ...
    models = {name: getattr(pipe, name) for name in pipe.in_iteration_models}
    if debug:
        print(f"[stage3c_dmd_debug] {probe_name} model_fn_start timestep={float(timestep.flatten()[0].item())}", flush=True)
...
def _stage3c_fake_fm_loss(
    ...
    *,
    ...
    if fake_weight == 0.0 or clean_latents is None or not isinstance(fake_model, FlashVSRStage3BTrainingModule):
        return None
    update_ratio = max(1, int(getattr(args, "stage3_fake_update_ratio", 1)))
    # Keep the first implementation conservative: one fake update for every
    # `update_ratio` student steps. More aggressive DMD2-style extra fake
    # substeps should be added only after the single-update path is stable.
...
def _maybe_run_stage3c_dmd_probe(real_model, fake_probe_model, data, clean_latents, args, global_step: int, *, dataset_load_from_cache: bool):
    ...
    if global_step % dmd_every != 0:
        return None
    with torch.no_grad():
        real_x0 = _stage3c_probe_predict_x0(real_model, data, clean_latents, dataset_load_from_cache=dataset_load_from_cache, probe_name="real_probe")
        fake_x0 = _stage3c_probe_predict_x0(fake_probe_model, data, clean_latents, dataset_load_from_cache=dataset_load_from_cache, probe_name="fake_probe")
...
def _maybe_run_stage3c_dmd_student_loss(
    ...
    *,
    ...
    if global_step % dmd_every != 0:
        return None, None, None
    with torch.no_grad():
        real_x0 = _stage3c_probe_predict_x0(real_model, data, clean_latents, dataset_load_from_cache=dataset_load_from_cache, probe_name="real_dmd")
        fake_x0 = _stage3c_probe_predict_x0(fake_probe_model, data, clean_latents, dataset_load_from_cache=dataset_load_from_cache, probe_name="fake_dmd")
        ...
        max_norm = float(getattr(args, "stage3_dmd_grad_norm_max", 0.0) or 0.0)
        spike_policy = str(getattr(args, "stage3_dmd_spike_policy", "none") or "none")
        dmd_skipped = torch.zeros((), device=clean_latents.device, dtype=torch.float32)
        if max_norm > 0.0 and float(dmd_grad_norm.detach().cpu()) > max_norm:
            if spike_policy == "skip":
                ...
                scale = max_norm / dmd_grad_norm.clamp_min(1e-6)
...
def launch_stage3c_dual_optimizer_task(
    ...
    """Dedicated v7-D1 runner skeleton with a second optimizer state path."""
    ...
    if wandb_run is None and getattr(args, "use_wandb", False) and accelerator.is_main_process:
        wandb_run = _init_stage3_wandb(args, accelerator=accelerator)
    ...
    if not fake_trainable_params:
        raise RuntimeError("Stage3 v7-D1 fake_model has no trainable parameters.")
    ...
    if getattr(args, "resume_training_state_dir", None):
        ...
        load_stage3c_extra_state_if_available(accelerator, args.resume_training_state_dir, fake_model, fake_optimizer, fake_scheduler)
        print(f"[stage3c_resume] loaded student/fake state step={global_step} epoch_id={resumed_epoch_id}", flush=True)

    if accelerator.is_main_process:
        print(
            "[stage3c_runner] C0 dual optimizer skeleton "
            ...
            f"fake_update_ratio={args.stage3_fake_update_ratio} "
            f"fake_trainable_params={_count_trainable_params(fake_model)}",
            flush=True,
        )
        if real_model is not None:
            print(
    ...
    for epoch_id in range(resumed_epoch_id, num_epochs):
        ...
        while True:
            ...
            if (
                getattr(args, "stage3_validation_from_training_batch", False)
                and model_logger.validation_callback is not None
                and accelerator.is_main_process
            ):
                extend_stage3_validation_samples_from_batch(
                    model_logger.validation_callback.validation_samples,
                    data,
                    num_samples=int(getattr(args, "validation_num_samples", 0) or 0),
                )
            ...
            with accelerator.accumulate(model):
                ...
                total_loss = student_loss + fake_loss
                if dmd_student_loss is not None:
                    total_loss = total_loss + dmd_student_loss
                accelerator.backward(total_loss)
                _clear_stage3c_last_z_pred(model)
                _average_stage3c_fake_gradients(fake_model)
                optimizer.step()
                fake_optimizer.step()
                should_save_checkpoint = _stage3c_should_save_next_step(model_logger, save_steps, extra_save_steps)
                requested_validation_samples = int(getattr(args, "validation_num_samples", 0) or 0)
                validation_ready = requested_validation_samples > 0
                if (
                    validation_ready
                    and getattr(args, "stage3_validation_from_training_batch", False)
                    and model_logger.validation_callback is not None
                ):
                    validation_ready = (
                        len(model_logger.validation_callback.validation_samples)
                        >= requested_validation_samples
                    )
                save_has_validation = should_save_checkpoint and validation_ready
                should_save_with_validation = model_logger.validation_callback is not None and save_has_validation
                validation_callback = None
                if should_save_checkpoint and model_logger.validation_callback is not None:
                    # Save first, then run validation manually only when enough
                    # rank0 training samples have been cached. ModelLogger would
                    # otherwise run validation unconditionally on every save.
                    validation_callback = model_logger.validation_callback
                ...
                if saved_checkpoint_path is not None:
                    ...
                    if validation_callback is not None and save_has_validation:
                    ...
                    if save_has_validation:
                        accelerator.wait_for_everyone()
                if validation_callback is not None and model_logger.validation_callback is None:
                    model_logger.validation_callback = validation_callback
                scheduler.step()
                fake_scheduler.step()
                global_step += 1
                ...
                if accelerator.is_main_process:
                    ...
                    fake_param = float(fake_model.scale.detach().cpu().item()) if isinstance(fake_model, Stage3CFakeScalarModel) else 0.0
                    ...
                    dmd_skipped_value = None
                    if dmd_skipped is not None:
                        dmd_skipped_value = float(dmd_skipped.detach().float().mean().item()) if no_gather_log else float(accelerator.gather(dmd_skipped.detach()).mean().item())
                    progress_bar.set_postfix(loss=f"{total_value:.6f}", step=global_step)
                    if log_loss_steps and global_step % log_loss_steps == 0:
                        ...
                        dmd_skip_text = "" if dmd_skipped_value is None else f" dmd_skip={dmd_skipped_value:.1f}"
                        print(
                            "[stage3c_train] "
                            f"epoch={epoch_id} step={global_step} "
                            f"loss={total_value:.6f} student={student_value:.6f} "
                            f"fake_loss={fake_value:.8f} fake_scale={fake_param:.6f}"
                    ...
                    if wandb_run is not None:
                        log_payload = {
                            ...
                            "train/fake_lr": float(fake_scheduler.get_last_lr()[0]),
                        }
                        ...
                        if dmd_skipped_value is not None:
                            log_payload["train/dmd_skipped"] = dmd_skipped_value
...
def _stage3_parser():
    ...
    parser.add_argument(
        "--stage3_validation_tiled",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use tiled VAE decode in Stage3 streaming validation. v7-D smoke keeps this false to test non-tiled parity.",
    )
    parser.add_argument(
        "--stage3_validation_collect_num_workers",
        type=int,
        default=0,
        help="CPU workers used only to pre-collect fixed Stage3 validation samples.",
    )
    parser.add_argument(
        "--stage3_validation_from_training_batch",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Cache rank0 training batch tensors for validation instead of pre-sampling the dataset before training.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Reserved for Stage3 v7-D1 full DMD. C0 keeps this 0 and only validates optimizer/state wiring.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional frozen G_real checkpoint for v7-D1 probe. This is no-grad and not part of optimizer.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional frozen G_fake probe checkpoint for v7-D1. This is logging-only and not optimized.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="If >0, guard DMD direction spikes when mean abs DMD grad exceeds this value.",
    )
    parser.add_argument(
        ...
        help="How to handle DMD grad spikes: none, skip the DMD term, or norm-clamp the DMD direction.",
    )
    return parser
...
def parse_stage3_args(argv=None):
    ...
    if args.num_frames % 8 != 1:
        parser.error("Stage3 v7-D1 currently follows Stage2 89->85 streaming shape, requiring num_frames % 8 == 1.")
...
def main(argv=None):
    ...
    model = FlashVSRStage3BTrainingModule(
        ...
        # Fake FM is handled by the dedicated v7-D1 runner and its second
        # optimizer. The student forward must keep the reconstruction guard disabled.
    ...
    if accelerator.is_local_main_process:
        ...
        print("Stage3 v7-D1: dual optimizer runner + streaming one-step validation path", flush=True)
    ...
    if args.validation_num_samples > 0 and accelerator.is_main_process:
        print(
            "Preparing fixed Stage3 v7-D1 validation samples... "
            f"num_samples={args.validation_num_samples} "
            f"collect_workers={args.stage3_validation_collect_num_workers} "
            f"from_training_batch={args.stage3_validation_from_training_batch}",
            flush=True,
        )
        if args.stage3_validation_from_training_batch:
            validation_samples = []
        else:
            validation_samples = collect_stage3_validation_samples(
                dataset,
                args.validation_num_samples,
                num_workers=max(0, int(args.stage3_validation_collect_num_workers)),
            )
        print(f"Prepared {len(validation_samples)} fixed Stage3 v7-D1 validation samples.", flush=True)
        validation_callback = FlashVSRStage3BValidationCallback(
            ...
            tiled=args.stage3_validation_tiled,
        )
    accelerator.wait_for_everyone()
    ...
    if args.stage3_real_checkpoint is not None:
        ...
        if accelerator.is_local_main_process:
            print(
                "Stage3 v7-D1 frozen G_real probe loaded "
    ...
    if args.stage3_fake_checkpoint is not None:
        ...
        if accelerator.is_local_main_process:
            print(
                "Stage3 v7-D1 trainable G_fake loaded "
    ...
    if args.stage3_fake_probe_checkpoint is not None:
        ...
        if accelerator.is_local_main_process:
            print(
                "Stage3 v7-D1 frozen G_fake probe loaded "
        ...
        if accelerator.is_local_main_process:
            print(
                "Stage3 v7-D1 uses trainable G_fake also as no-grad DMD fake probe.",
                flush=True,
            )
