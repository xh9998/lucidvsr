...
from diffsynth.models.wan_video_dit_joint_v1 import (
    _build_segment_token_lengths,
    build_joint_wan_from_existing,
)
...
def _resolve_per_sample_token_lengths(
    ...
    sequence_lengths_list = [_to_latent_frames(int(length)) for length in sequence_lengths.detach().cpu().tolist()]
...
class FlashVSRStage1FullFinetuneTrainingModule(DiffusionTrainingModule):
    def __init__(
        ...
        if image_video_joint_packed:
            self.pipe.dit = build_joint_wan_from_existing(self.pipe.dit)
...
def flashvsr_v3_parser():
    ...
    parser.add_argument("--dataset_mode", type=str, default="unified", choices=("unified", "streaming", "parquet_v2", "tar_v3"), help="Dataset backend.")
    ...
    parser.add_argument("--picked17k_image_tar_url", type=str, default=None, help="picked17k image tar root for tar_v3 mode.")
    parser.add_argument("--picked17k_dataset_prob", type=float, default=0.1, help="Sampling probability of picked17k image source in tar_v3 mode.")
    ...
    parser.add_argument("--image_video_joint_packed", default=False, action="store_true", help="Enable packed varlen joint attention for mixed image/video parquet_v2 batches.")
...
def main():
    ...
    if args.dataset_mode == "streaming":
        ...
        if args.image_video_joint_packed:
            dataset.custom_collate_fn = collate_image_video_joint_v1
        else:
