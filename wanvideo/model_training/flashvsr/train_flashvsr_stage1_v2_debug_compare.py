    ...
    try:
        ...
        if tensor is not None:
            ...
            try:
                ...
                with open(os.path.join(debug_dir, f"{key}.video_skip.txt"), "w", encoding="utf-8") as file:
                    file.write(str(video_error))
...
class WanFixedPromptEmbeddedUnit(PipelineUnit):
    ...
    def process(self, pipe):
        ...
        _dump_tensor_preview_once("00_fixed_prompt_raw", raw_context, pipe=pipe)
        embedded_context = pipe.dit.text_embedding(raw_context)
        _dump_tensor_preview_once("00_embedded_context", embedded_context, pipe=pipe)
...
class WanFixedPromptFlashVSRStage1Pipeline(WanVideoPipeline):
    ...
    def infer_from_lq(
        ...
        if "latents" not in inputs_shared:
            inputs_shared["latents"] = inputs_shared["noise"]
        if "noise" in inputs_shared and torch.is_tensor(inputs_shared["noise"]):
            _dump_tensor_preview_once("10_noise_init", inputs_shared["noise"], pipe=self)
        if "latents" in inputs_shared and torch.is_tensor(inputs_shared["latents"]):
            _dump_tensor_preview_once("11_latents_init", inputs_shared["latents"], pipe=self)
        ...
        for progress_id, timestep in enumerate(self.scheduler.timesteps):
            timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device)
            noise_pred = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep)
            if progress_id == 0 and torch.is_tensor(noise_pred):
                _dump_tensor_preview_once(
                    "12_step0_noise_pred",
                    noise_pred,
                    pipe=self,
                    extra={"timestep": float(self.scheduler.timesteps[progress_id].item())},
                )
            inputs_shared["latents"] = self.scheduler.step(
                noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"]
            )
            if progress_id == 0 and torch.is_tensor(inputs_shared["latents"]):
                _dump_tensor_preview_once(
                    "13_step0_latents_after_step",
                    inputs_shared["latents"],
                    pipe=self,
                    extra={"timestep": float(self.scheduler.timesteps[progress_id].item())},
                )
        ...
        try:
            for sample_index, sample in enumerate(self.validation_samples):
                ...
                if self.validation_use_wan_text_baseline:
                    ...
                    if _tensor_debug_dir(pipe):
                        _TENSOR_DEBUG_REPORTED.clear()
                        baseline_pipe.debug_tensor_dump_dir = os.path.join(sample_dir, "tensor_debug")
