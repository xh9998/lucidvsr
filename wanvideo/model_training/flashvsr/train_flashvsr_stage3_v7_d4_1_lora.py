...
def save_stage3c_extra_state(accelerator, output_path: str, step: int, fake_model, fake_optimizer, fake_scheduler, args):
    ...
    fake_module = _unwrap_stage3c_model(fake_model)
    fake_is_full_model = isinstance(fake_module, FlashVSRStage3BTrainingModule)
    if fake_is_full_model:
        fake_state = fake_module.export_trainable_state_dict(fake_module.state_dict())
        fake_state_note = "v7-C trainable G_fake state: trainable LoRA/lq_proj parameters only."
    else:
        fake_state = fake_module.state_dict()
    ...
    payload = {
        ...
        "stage3_dfake_gen_update_ratio": int(_stage3d41_dfake_gen_update_ratio(args)),
...
def load_stage3c_extra_state_if_available(accelerator, state_dir: str, fake_model, fake_optimizer, fake_scheduler):
    ...
    fake_module = _unwrap_stage3c_model(fake_model)
    if isinstance(fake_module, FlashVSRStage3BTrainingModule):
        result = fake_module.load_state_dict(fake_state, strict=False)
        if accelerator.is_main_process:
            print(
        ...
        fake_module.load_state_dict(fake_state, strict=True)
...
def _stage3d41_wrap_fake_ddp_if_needed(fake_model: torch.nn.Module, accelerator):
    if not (dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1):
        return fake_model
    for param in fake_model.parameters():
        dist.broadcast(param.data, src=0)
    for buffer in fake_model.buffers():
...
def _stage3d41_student_z_pred_only_forward(model, data, *, dataset_load_from_cache: bool):
    """Run the student only far enough to produce z_pred for fake FM training.

    Fake-only turns must not pay the Stage3 pixel/LPIPS reconstruction cost; the
    fake critic only needs the current detached student latent distribution.
    """
    module = _unwrap_stage3c_model(model)
    old_mse = getattr(module, "stage3_mse_weight", 0.0)
    old_lpips = getattr(module, "stage3_lpips_weight", 0.0)
    ...
    try:
        ...
        module.stage3_mse_weight = old_mse
        module.stage3_lpips_weight = old_lpips
...
def _stage3d41_dfake_gen_update_ratio(args) -> int:
    return max(1, int(getattr(args, "stage3_dfake_gen_update_ratio", 1)))


def _stage3d41_is_generator_turn(args, runner_step: int) -> bool:
    ratio = _stage3d41_dfake_gen_update_ratio(args)
    # Follow DMD2's runner semantics: guidance/fake is trained every iteration,
    # while generator/student is trained once every `ratio` iterations.
    return int(runner_step) % ratio == 0
...
def launch_stage3c_dual_optimizer_task(
    ...
    fake_model = _stage3d41_wrap_fake_ddp_if_needed(fake_model, accelerator)
    ...
    if getattr(args, "resume_training_state_dir", None):
        ...
        dfake_ratio = _stage3d41_dfake_gen_update_ratio(args)
        runner_step = global_step * dfake_ratio
    ...
    if accelerator.is_main_process:
        print(
            ...
            f"dfake_gen_update_ratio={_stage3d41_dfake_gen_update_ratio(args)} "
            "dmd2_alternating=1 "
            f"fake_ddp={int(isinstance(fake_model, DDP))} "
            f"fake_manual_grad_sync={int(dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1)} "
    ...
    for epoch_id in range(resumed_epoch_id, num_epochs):
        ...
        while True:
            ...
            sync_debug = os.environ.get("FLASHVSR_STAGE3_SYNC_DEBUG", "0") == "1"
            timing_last = _stage3_timing_now(accelerator.device) if timing_debug else None
            timing_parts = {}
            if sync_debug and dist.is_available() and dist.is_initialized():
                print(
                    f"[stage3_sync] rank={dist.get_rank()} runner_step={runner_step} before_next_data",
                    flush=True,
                )
            try:
            ...
            if sync_debug and dist.is_available() and dist.is_initialized():
                print(
                    f"[stage3_sync] rank={dist.get_rank()} runner_step={runner_step} after_next_data",
                    flush=True,
                )
            if is_iterable_dataset:
            ...
            generator_turn = _stage3d41_is_generator_turn(args, current_runner_step)
            if sync_debug and dist.is_available() and dist.is_initialized():
                print(
                    f"[stage3_sync] rank={dist.get_rank()} runner_step={current_runner_step} "
                    f"turn={'generator+fake' if generator_turn else 'fake'} enter_turn",
                    flush=True,
                )
            with accelerator.accumulate(model):
                ...
                if generator_turn:
                    ...
                    student_loss = _stage3d41_student_z_pred_only_forward(
                        model,
                        data,
                ...
                if timing_debug:
                    ...
                    timing_last = now
                real_probe_loss = None
                fake_probe_loss = None
                if generator_turn:
                ...
                if generator_turn:
                    ...
                    if dist.is_available() and dist.is_initialized() and dist.get_world_size() > 1:
                        # Stage3 data/decode/offload can skew ranks by minutes.
                        # Align ranks before entering the student DeepSpeed
                        # ZeRO2 gradient allreduce, whose NCCL watchdog remains
                        # shorter than the main process-group timeout.
                        if os.environ.get("FLASHVSR_STAGE3_SYNC_DEBUG", "0") == "1":
                            print(
                                f"[stage3_sync] rank={dist.get_rank()} runner_step={current_runner_step} "
                ...
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                    timing_parts["generator_backward"] = now - timing_last
                    timing_last = now
                if generator_turn:
                    optimizer.step()
                    optimizer.zero_grad()
                ...
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                    timing_parts["generator_optim"] = now - timing_last
                    timing_last = now

                # DMD2 runner semantics: one optional generator phase followed
                # by one guidance/fake phase. The fake phase consumes only a
                # detached student sample, so it never shares a backward graph
                # with the student optimizer.
                fake_did_update = False
                fake_loss = torch.zeros((), device=accelerator.device, dtype=torch.float32)
                fake_module = _unwrap_stage3c_model(fake_model)
                if isinstance(fake_module, Stage3CFakeScalarModel):
                ...
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                    timing_parts["fake_forward"] = now - timing_last
                    timing_last = now
                if fake_did_update:
                    # G_fake is deliberately outside the student DeepSpeed engine.
                    # Calling accelerator.backward(fake_loss) would route the loss
                    # through the student ZeRO2 engine, whose buckets do not own
                    # fake params. We therefore keep fake backward isolated and
                    # explicitly average fake gradients after autograd finishes.
                    fake_loss.backward()
                ...
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                    timing_parts["fake_backward_optim"] = now - timing_last
                    timing_last = now
                ...
                if accelerator.is_main_process:
                    ...
                    fake_module = _unwrap_stage3c_model(fake_model)
                    fake_param = float(fake_module.scale.detach().cpu().item()) if isinstance(fake_module, Stage3CFakeScalarModel) else 0.0
                    ...
                    if log_loss_steps and current_runner_step % log_loss_steps == 0:
                        ...
                        print(
                            ...
                            f"turn={'generator+fake' if generator_turn else 'fake'} "
                            f"generator_update={int(generator_turn)} dfake_gen_update_ratio={_stage3d41_dfake_gen_update_ratio(args)} "
                    ...
                    if wandb_run is not None:
                        log_payload = {
                            ...
                            "train/turn_is_fake": int(not generator_turn),
                            "train/fake_scale": fake_param,
                            "train/step": global_step,
                            "train/runner_step": current_runner_step,
                            "train/dfake_gen_update_ratio": _stage3d41_dfake_gen_update_ratio(args),
                            "train/fake_ddp": int(isinstance(fake_model, DDP)),
                            "train/fake_manual_grad_sync": int(
...
def _stage3_parser():
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "v7-D4.1 DMD2-style dfake/gen ratio. G_fake is updated every runner "
            "step; student/generator is updated when runner_step % ratio == 0. "
            "Larger values mean fewer student updates per fake critic update."
        ),
    )
...
def main(argv=None):
    ...
    dist_timeout_seconds = max(60, int(os.environ.get("FLASHVSR_DIST_TIMEOUT_SECONDS", "7200")))
    accelerator_kwargs = {
        "gradient_accumulation_steps": args.gradient_accumulation_steps,
        "kwargs_handlers": [
            accelerate.DistributedDataParallelKwargs(find_unused_parameters=args.find_unused_parameters),
            accelerate.InitProcessGroupKwargs(timeout=timedelta(seconds=dist_timeout_seconds)),
        ],
    }
    ...
    if accelerator.is_local_main_process:
        ...
        print(f"distributed_timeout_seconds={dist_timeout_seconds}", flush=True)
        print(
            "stage3c_real "
            f"checkpoint={args.stage3_real_checkpoint} "
            f"attention_mode={args.stage3_real_attention_mode} "
    ...
    elif isinstance(_unwrap_stage3c_model(fake_model), FlashVSRStage3BTrainingModule):
