...
class FlashVSRUnit_LQVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe, lq_video, height, width):
        ...
        if getattr(pipe.lq_proj_in, "layer_num", None) == 1:
...
class FlashVSRValidationCallback:
    def __init__(
        ...
        self._wan_text_baseline_pipe = None

    def _get_wan_text_baseline_pipe(self, device, torch_dtype):
        if self._wan_text_baseline_pipe is None:
            self._wan_text_baseline_pipe = WanVideoPipeline.from_pretrained(
                torch_dtype=torch_dtype,
                device=device,
                model_configs=self.validation_model_configs,
                tokenizer_config=self.validation_tokenizer_config,
            )
        return self._wan_text_baseline_pipe
                    ...
                    json.dump(
                        {
                            ...
                            "validation_use_wan_text_baseline": self.validation_use_wan_text_baseline,
...
def main():
    ...
    if args.validation_num_samples > 0 and accelerator.is_main_process:
        ...
        if args.validation_use_wan_text_baseline:
            ...
            if not model_paths:
                raise ValueError("validation_use_wan_text_baseline requires model_paths to locate the base Wan model.")
