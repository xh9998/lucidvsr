...
def Stage3BOneStepReconLoss(pipe, *, stage3_recon_num_latents: int, stage3_flow_weight: float,
    ...
    if (
        os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_DEBUG", "0") == "1"
        or os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_SUBSET", "0") == "1"
    ):
        pipe._stage3_last_loss_tensors = {
            "flow": float(stage3_flow_weight) * flow_loss,
            "mse": float(stage3_mse_weight) * mse_loss,
            "lpips": float(stage3_lpips_weight) * lpips_loss,
            "recon": float(stage3_mse_weight) * mse_loss + float(stage3_lpips_weight) * lpips_loss,
            "student_no_dmd": total,
        }
    if os.environ.get("FLASHVSR_STAGE3_GRAD_SCALE_DEBUG", "0") == "1" and os.environ.get("LOCAL_RANK", "0") == "0":
        weighted_flow = float(stage3_flow_weight) * flow_loss
        weighted_mse = float(stage3_mse_weight) * mse_loss
        weighted_lpips = float(stage3_lpips_weight) * lpips_loss
        weighted_recon = weighted_mse + weighted_lpips
        flow_noise_stats = _stage3_tensor_grad_stats(weighted_flow, noise_pred)
        flow_z_stats = _stage3_tensor_grad_stats(weighted_flow, z_pred)
        mse_z_stats = _stage3_tensor_grad_stats(weighted_mse, z_pred)
        lpips_z_stats = _stage3_tensor_grad_stats(weighted_lpips, z_pred)
        recon_z_stats = _stage3_tensor_grad_stats(weighted_recon, z_pred)
        print(
            "[stage3_grad_scale_recon] "
            f"weights=flow:{float(stage3_flow_weight):.4g},mse:{float(stage3_mse_weight):.4g},lpips:{float(stage3_lpips_weight):.4g} "
            f"loss_flow={float(flow_loss.detach().float().mean().cpu().item()):.8f} "
            f"loss_mse={float(mse_loss.detach().float().mean().cpu().item()):.8f} "
            f"loss_lpips={float(lpips_loss.detach().float().mean().cpu().item()):.8f} "
            f"{_stage3_format_grad_stats('flow_to_noise_pred', flow_noise_stats)} "
            f"{_stage3_format_grad_stats('flow_to_z_pred', flow_z_stats)} "
            f"{_stage3_format_grad_stats('mse_to_z_pred', mse_z_stats)} "
            f"{_stage3_format_grad_stats('lpips_to_z_pred', lpips_z_stats)} "
            f"{_stage3_format_grad_stats('recon_to_z_pred', recon_z_stats)}",
            flush=True,
        )
...
def _stage3_grad_norm(model: Optional[torch.nn.Module]) -> float:
    if model is None:
        return 0.0
    total = 0.0
    for param in model.parameters():
...
def _stage3_tensor_grad_stats(loss: Optional[torch.Tensor], tensor: Optional[torch.Tensor]) -> Dict[str, float]:
    if loss is None or tensor is None or not torch.is_tensor(loss) or not torch.is_tensor(tensor):
        return {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": 0.0}
    if not loss.requires_grad or not tensor.requires_grad:
        return {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": float(tensor.numel())}
    try:
        ...
        if os.environ.get("LOCAL_RANK", "0") == "0":
            print(f"[stage3_grad_scale_error] {type(error).__name__}: {error}", flush=True)
        return {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": float(tensor.numel())}
    if grad is None:
        return {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": float(tensor.numel())}
    grad = grad.detach().float()
    return {
        "mean_abs": float(grad.abs().mean().cpu().item()),
        "l2": float(grad.pow(2).sum().sqrt().cpu().item()),
        "max_abs": float(grad.abs().max().cpu().item()),
        "numel": float(grad.numel()),
    }


def _stage3_param_group_name(name: str) -> str:
    if "lora_" in name:
        return "lora"
    if "lq_proj_in" in name:
        return "lq_proj_in"
    return "other"


def _stage3_param_grad_stats(loss: Optional[torch.Tensor], model: Optional[torch.nn.Module]) -> Dict[str, Dict[str, float]]:
    zero = {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0}
    empty = {"all": dict(zero), "lora": dict(zero), "lq_proj_in": dict(zero), "other": dict(zero)}
    if loss is None or model is None or not torch.is_tensor(loss) or not loss.requires_grad:
        return empty
    ...
    try:
        ...
        if os.environ.get("LOCAL_RANK", "0") == "0":
            print(f"[stage3_param_grad_error] backward {type(error).__name__}: {error}", flush=True)
        try:
            model.zero_grad(set_to_none=True)
        except Exception:
            pass
    ...
    accum: Dict[str, Dict[str, float]] = {
        "all": {"sum_abs": 0.0, "sum_sq": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0},
        "lora": {"sum_abs": 0.0, "sum_sq": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0},
        "lq_proj_in": {"sum_abs": 0.0, "sum_sq": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0},
        "other": {"sum_abs": 0.0, "sum_sq": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0},
    }
    for name, param in named_params:
        ...
        if grad is None:
            continue
        grad = grad.detach().float()
        group = _stage3_param_group_name(name)
        sum_abs = float(grad.abs().sum().cpu().item())
        sum_sq = float(grad.pow(2).sum().cpu().item())
        max_abs = float(grad.abs().max().cpu().item()) if grad.numel() else 0.0
        numel = float(grad.numel())
        for key in ("all", group):
            accum[key]["sum_abs"] += sum_abs
            accum[key]["sum_sq"] += sum_sq
            accum[key]["max_abs"] = max(accum[key]["max_abs"], max_abs)
            accum[key]["numel"] += numel
            accum[key]["used_params"] += 1.0
    model.zero_grad(set_to_none=True)

    result: Dict[str, Dict[str, float]] = {}
    for key, values in accum.items():
        result[key] = {
            "mean_abs": values["sum_abs"] / max(values["numel"], 1.0),
            "l2": values["sum_sq"] ** 0.5,
            "max_abs": values["max_abs"],
            "numel": values["numel"],
            "used_params": values["used_params"],
        }
    return result


def _stage3_format_grad_stats(prefix: str, stats: Dict[str, float]) -> str:
    return (
        f"{prefix}_mean_abs={stats['mean_abs']:.6e} "
        f"{prefix}_l2={stats['l2']:.6e} "
        f"{prefix}_max_abs={stats['max_abs']:.6e} "
        f"{prefix}_numel={int(stats['numel'])}"
    )


def _stage3_format_param_grad_stats(prefix: str, stats_by_group: Dict[str, Dict[str, float]]) -> str:
    parts = []
    for group in ("all", "lora", "lq_proj_in", "other"):
        stats = stats_by_group.get(group, {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0})
        parts.append(
            f"{prefix}_{group}_mean_abs={stats['mean_abs']:.6e} "
            f"{prefix}_{group}_l2={stats['l2']:.6e} "
            f"{prefix}_{group}_max_abs={stats['max_abs']:.6e} "
            f"{prefix}_{group}_numel={int(stats['numel'])} "
            f"{prefix}_{group}_used_params={int(stats['used_params'])}"
        )
    return " ".join(parts)


def _stage3_current_param_grad_stats(model: Optional[torch.nn.Module]) -> Dict[str, Dict[str, float]]:
    zero = {"mean_abs": 0.0, "l2": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0}
    if model is None:
        return {"all": dict(zero), "lora": dict(zero), "lq_proj_in": dict(zero), "other": dict(zero)}
    safe_get_full_grad = None
    safe_get_local_grad = None
    try:
        from deepspeed.utils import safe_get_full_grad as _safe_get_full_grad
        safe_get_full_grad = _safe_get_full_grad
    except Exception:
        safe_get_full_grad = None
    try:
        from deepspeed.utils import safe_get_local_grad as _safe_get_local_grad
        safe_get_local_grad = _safe_get_local_grad
    except Exception:
        safe_get_local_grad = None
    accum: Dict[str, Dict[str, float]] = {
        "all": {"sum_abs": 0.0, "sum_sq": 0.0, "max_abs": 0.0, "numel": 0.0, "used_params": 0.0},
    ...
    # Diagnostic copy only: ZeRO2 safe grad reads are expensive, so sample a
    # stable prefix of each trainable group instead of scanning the full model.
    limits = {
        "lora": max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_STATS_MAX_LORA_PARAMS", "12"))),
        "lq_proj_in": max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_STATS_MAX_LQ_PARAMS", "2"))),
        "other": max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_STATS_MAX_OTHER_PARAMS", "0"))),
    }
    seen = {"lora": 0, "lq_proj_in": 0, "other": 0}
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        group = _stage3_param_group_name(name)
        if seen[group] >= limits[group]:
            continue
        seen[group] += 1
        grad = param.grad
        if (
            os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_USE_LOCAL_SAFE", "1") == "1"
            and safe_get_local_grad is not None
        ):
            try:
...
def _stage3_subset_param_grad_stats(
    ...
    max_lora = max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_SUBSET_LORA_PARAMS", "8")))
    max_lq = max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_SUBSET_LQ_PARAMS", "1")))
    max_other = max(0, int(os.environ.get("FLASHVSR_STAGE3_PARAM_SUBSET_OTHER_PARAMS", "0")))
    limits = {"lora": max_lora, "lq_proj_in": max_lq, "other": max_other}
    counts = {"lora": 0, "lq_proj_in": 0, "other": 0}
    selected: List[Tuple[str, str, torch.nn.Parameter]] = []
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        group = _stage3_param_group_name(name)
        if counts[group] >= limits[group]:
            continue
        selected.append((name, group, param))
        counts[group] += 1
    ...
    params = [param for _, _, param in selected]
    try:
        ...
        if os.environ.get("LOCAL_RANK", "0") == "0":
            print(f"[stage3_param_grad_subset_error] {type(error).__name__}: {error}", flush=True)
        return empty, ",".join(name for name, _, _ in selected)
    ...
    for (_, group, _), grad in zip(selected, grads):
        if grad is None:
            continue
        grad = grad.detach().float()
        sum_abs = float(grad.abs().sum().cpu().item())
    ...
    names = ",".join(name for name, _, _ in selected)
    return result, names


def _stage3_param_checksum(model: Optional[torch.nn.Module], *, trainable_only: bool = True) -> float:
    if model is None:
        return 0.0
    total = 0.0
    for param in model.parameters():
        ...
        if param.numel() == 0:
            continue
        total += float(param.detach().float().sum().cpu().item())
    return total


def _maybe_log_stage3_grad_ownership(
    accelerator,
    *,
    runner_step: int,
    global_step: int,
    ...
    fake_model: Optional[torch.nn.Module],
    real_model: Optional[torch.nn.Module],
    total_loss: torch.Tensor,
    student_loss: torch.Tensor,
    fake_loss: torch.Tensor,
    dmd_student_loss: Optional[torch.Tensor],
) -> None:
    if os.environ.get("FLASHVSR_STAGE3_GRAD_OWNERSHIP_DEBUG", "0") != "1":
        return
    if not accelerator.is_main_process:
        return
    dmd_value = "None"
    if dmd_student_loss is not None:
        dmd_value = f"{float(dmd_student_loss.detach().float().mean().cpu().item()):.8f}"
    print(
        "[stage3_grad_ownership] "
        f"runner_step={int(runner_step)} step={int(global_step)} turn={turn} "
        f"student_grad_norm={_stage3_grad_norm(model):.6e} "
        f"fake_grad_norm={_stage3_grad_norm(fake_model):.6e} "
        f"real_grad_norm={_stage3_grad_norm(real_model):.6e} "
        f"total_loss={float(total_loss.detach().float().mean().cpu().item()):.8f} "
        f"student_loss={float(student_loss.detach().float().mean().cpu().item()):.8f} "
        f"fake_loss={float(fake_loss.detach().float().mean().cpu().item()):.8f} "
        f"dmd_student={dmd_value}",
        flush=True,
    )


def _maybe_log_stage3_param_ownership(
    accelerator,
    *,
    runner_step: int,
    global_step: int,
    ...
    if not accelerator.is_main_process:
        return
    after = {
        "student": _stage3_param_checksum(model, trainable_only=True),
        "fake": _stage3_param_checksum(fake_model, trainable_only=True),
        "real": _stage3_param_checksum(real_model, trainable_only=False),
    }
    print(
        "[stage3_param_ownership] "
        f"runner_step={int(runner_step)} step={int(global_step)} turn={turn} "
        f"student_delta={after['student'] - before['student']:.6e} "
        f"fake_delta={after['fake'] - before['fake']:.6e} "
        f"real_delta={after['real'] - before['real']:.6e} "
        f"student_before={before['student']:.6e} student_after={after['student']:.6e} "
        f"fake_before={before['fake']:.6e} fake_after={after['fake']:.6e} "
        f"real_before={before['real']:.6e} real_after={after['real']:.6e}",
        flush=True,
    )


def _apply_stage3_fake_trainable_filter(model: torch.nn.Module) -> None:
    mode = os.environ.get("FLASHVSR_STAGE3_FAKE_TRAINABLE_FILTER", "").strip().lower()
    if not mode or mode in {"all", "default"}:
        return
    if mode not in {"lora_only", "lq_proj_only"}:
        raise ValueError(f"Unsupported FLASHVSR_STAGE3_FAKE_TRAINABLE_FILTER={mode!r}")
    for name, param in model.named_parameters():
        if mode == "lora_only":
            param.requires_grad = "lora_" in name
        elif mode == "lq_proj_only":
            param.requires_grad = name.startswith("pipe.lq_proj_in.")
...
def _stage3d44_load_deepspeed_config(args, *, micro_batch_size: int, fake: bool) -> Dict[str, Any]:
    ...
    config_path = None
    if plugin is None:
        config_path = getattr(args, "deepspeed_config_file", None)
...
def _stage3d44_select_deepspeed_plugin(accelerator, name: str) -> None:
    if os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_DISABLE_DS", "0") == "1":
        return
...
def _get_stage3c_last_loss_tensors(model):
    module = _unwrap_stage3c_model(model)
    pipe = getattr(module, "pipe", None)
    return {} if pipe is None else dict(getattr(pipe, "_stage3_last_loss_tensors", {}) or {})
...
def _clear_stage3c_last_z_pred(model) -> None:
    ...
    if pipe is not None and hasattr(pipe, "_stage3_last_loss_tensors"):
        pipe._stage3_last_loss_tensors = {}
...
def _maybe_run_stage3c_dmd_student_loss(
    ...
    *,
    ...
    return dmd_loss * dmd_weight, dmd_grad_norm, dmd_skipped
        ...
        while True:
            ...
            with accelerator.accumulate(model):
                ownership_before = {
                    ...
                    "fake": _stage3_param_checksum(fake_model, trainable_only=True),
                    "real": _stage3_param_checksum(real_model, trainable_only=False),
                } if os.environ.get("FLASHVSR_STAGE3_GRAD_OWNERSHIP_DEBUG", "0") == "1" else None
                optimizer.zero_grad()
                ...
                if timing_debug:
                    ...
                    timing_last = now
                dmd_student_loss, dmd_grad_norm, dmd_skipped = _maybe_run_stage3c_dmd_student_loss(
                    real_model,
                    fake_probe_model,
                    data,
                    student_z_pred,
                    ...
                    global_step,
                    dataset_load_from_cache=bool(dataset.load_from_cache),
                ) if generator_turn else (None, None, None)
                if (
                    generator_turn
                    and dmd_student_loss is not None
                    and os.environ.get("FLASHVSR_STAGE3_GRAD_SCALE_DEBUG", "0") == "1"
                    and accelerator.is_main_process
                ):
                    dmd_z_stats = _stage3_tensor_grad_stats(dmd_student_loss, student_z_pred)
                    print(
                        "[stage3_grad_scale_dmd] "
                        f"runner_step={int(current_runner_step)} step={int(global_step)} "
                        f"loss_dmd_weighted={float(dmd_student_loss.detach().float().mean().cpu().item()):.8f} "
                        f"dmd_raw_grad_mean_abs={float(dmd_grad_norm.detach().float().mean().cpu().item()) if dmd_grad_norm is not None else 0.0:.6e} "
                        f"dmd_skipped={float(dmd_skipped.detach().float().mean().cpu().item()) if dmd_skipped is not None else 0.0:.1f} "
                        f"{_stage3_format_grad_stats('dmd_to_z_pred', dmd_z_stats)}",
                        flush=True,
                    )
                if (
                    generator_turn
                    and os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_DEBUG", "0") == "1"
                    and accelerator.is_main_process
                ):
                    student_module = _unwrap_stage3c_model(model)
                    loss_tensors = _get_stage3c_last_loss_tensors(model)
                    param_grad_items = [
                        ("flow", loss_tensors.get("flow")),
                        ("mse", loss_tensors.get("mse")),
                        ("lpips", loss_tensors.get("lpips")),
                        ("recon", loss_tensors.get("recon")),
                        ("student_no_dmd", loss_tensors.get("student_no_dmd")),
                        ("dmd", dmd_student_loss),
                    ]
                    for loss_name, loss_tensor in param_grad_items:
                        stats = _stage3_param_grad_stats(loss_tensor, student_module)
                        print(
                            "[stage3_param_grad_scale] "
                            f"runner_step={int(current_runner_step)} step={int(global_step)} "
                            f"loss_name={loss_name} "
                            f"loss_value={float(loss_tensor.detach().float().mean().cpu().item()) if torch.is_tensor(loss_tensor) else 0.0:.8f} "
                            f"{_stage3_format_param_grad_stats(loss_name, stats)}",
                            flush=True,
                        )
                if generator_turn and os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_SUBSET", "0") == "1":
                    loss_label = os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_LABEL", "student_total")
                    student_module = _unwrap_stage3c_model(model)
                    loss_tensors = _get_stage3c_last_loss_tensors(model)
                    subset_loss = dmd_student_loss if loss_label == "dmd" else loss_tensors.get(loss_label)
                    if subset_loss is None and loss_label == "student_total":
                        subset_loss = student_loss if dmd_student_loss is None else student_loss + dmd_student_loss
                    stats, selected_names = _stage3_subset_param_grad_stats(subset_loss, student_module)
                    if accelerator.is_main_process:
                        print(
                            "[stage3_param_grad_subset] "
                            f"runner_step={int(current_runner_step)} step={int(global_step)} "
                            f"loss_name={loss_label} "
                            f"loss_value={float(subset_loss.detach().float().mean().cpu().item()) if torch.is_tensor(subset_loss) else 0.0:.8f} "
                            f"{_stage3_format_param_grad_stats(loss_label, stats)} "
                            f"selected_params={selected_names}",
                            flush=True,
                        )
                if timing_debug:
                    now = _stage3_timing_now(accelerator.device)
                ...
                if generator_turn and dmd_student_loss is None:
                    ...
                    if timing_debug:
                        ...
                        timing_last = now
                skip_fake_for_param_grad = (
                    os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_SKIP_FAKE_WHEN_ZERO", "0") == "1"
                    and float(getattr(args, "stage3_fake_fm_weight", 0.0) or 0.0) == 0.0
                    and float(getattr(args, "stage3c_fake_skeleton_loss_weight", 0.0) or 0.0) == 0.0
                )
                fake_did_update = True
                if skip_fake_for_param_grad:
                    fake_did_update = False
                    fake_loss = torch.zeros((), device=accelerator.device, dtype=torch.float32)
                elif isinstance(fake_model, Stage3CFakeScalarModel):
                ...
                if generator_turn:
                    ...
                    if os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_AFTER_BACKWARD", "0") == "1":
                        loss_label = os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_LABEL", "student_total")
                        student_module = _unwrap_stage3c_model(model)
                        stats = _stage3_current_param_grad_stats(student_module)
                        if accelerator.is_main_process:
                            print(
                                "[stage3_param_grad_after_backward] "
                                f"runner_step={int(current_runner_step)} step={int(global_step)} "
                                f"loss_name={loss_label} "
                                f"student_total_loss={float(student_total_loss.detach().float().mean().cpu().item()):.8f} "
                                f"student_loss={float(student_loss.detach().float().mean().cpu().item()):.8f} "
                                f"dmd_loss={float(dmd_student_loss.detach().float().mean().cpu().item()) if torch.is_tensor(dmd_student_loss) else 0.0:.8f} "
                                f"{_stage3_format_param_grad_stats(loss_label, stats)}",
                                flush=True,
                            )
                    if timing_debug:
                ...
                _maybe_log_stage3_grad_ownership(
                    accelerator,
                    runner_step=current_runner_step,
                    global_step=global_step,
                    turn="generator" if generator_turn else "fake",
                    model=model,
                    ...
                    fake_loss=fake_loss,
                    dmd_student_loss=dmd_student_loss,
                )
                ...
                if ownership_before is not None:
                    _maybe_log_stage3_param_ownership(
                        accelerator,
                        runner_step=current_runner_step,
                        global_step=global_step,
                        turn="generator" if generator_turn else "fake",
                ...
                if accelerator.is_main_process:
                    ...
                    if log_loss_steps and current_runner_step % log_loss_steps == 0:
                        ...
                        print(
                            ...
                            f"fake_loss={fake_value:.8f} fake_update={int(fake_did_update)} fake_scale={fake_param:.6f}"
                            f"{real_probe_text}{fake_probe_text}{dmd_probe_text}"
                            f"{dmd_student_text}{dmd_grad_text}{dmd_skip_text}",
                            flush=True,
                        )
                        if timing_debug:
                    ...
                    if wandb_run is not None:
                        log_payload = {
                            ...
                            "train/student_loss": student_value,
                            "train/fake_skeleton_loss": fake_value,
                            "train/fake_did_update": int(fake_did_update),
                            "train/generator_did_update": int(generator_turn),
                            "train/fake_scale": fake_param,
                            "train/step": global_step,
                            "train/runner_step": current_runner_step,
                            "train/dfake_gen_update_ratio": _stage3d4_dfake_gen_update_ratio(args),
                            "train/epoch": epoch_id,
                            "train/lr": float(scheduler.get_last_lr()[0]),
                            "train/fake_lr": float(fake_scheduler.get_last_lr()[0]) if hasattr(fake_scheduler, "get_last_lr") else fake_learning_rate,
                        }
                        if real_probe_value is not None:
                            log_payload["train/real_probe_loss"] = real_probe_value
                        if fake_probe_value is not None:
                            log_payload["train/fake_probe_loss"] = fake_probe_value
                        if dmd_probe_value is not None:
                            log_payload["train/dmd_probe_loss"] = dmd_probe_value
                        if dmd_student_value is not None:
                            log_payload["train/dmd_student_loss"] = dmd_student_value
                        if dmd_grad_value is not None:
                            log_payload["train/dmd_grad_norm"] = dmd_grad_value
                        if dmd_skip_value is not None:
                            log_payload["train/dmd_skipped"] = dmd_skip_value
...
def main(argv=None):
    ...
    if os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_NO_ACTCKPT", "0") == "1":
    ...
    disable_param_grad_ds = os.environ.get("FLASHVSR_STAGE3_PARAM_GRAD_DISABLE_DS", "0") == "1"
    deepspeed_plugins = None if disable_param_grad_ds else _stage3d44_build_deepspeed_plugins(args, micro_batch_size=max(1, int(args.batch_size)))
    if deepspeed_plugins is not None:
        accelerator_kwargs["deepspeed_plugins"] = deepspeed_plugins
    ...
    if args.stage3_fake_checkpoint is not None:
        ...
        _apply_stage3_fake_trainable_filter(fake_model)
        if accelerator.is_local_main_process:
            print(
                "Stage3 v7-C6 trainable G_fake loaded "
                f"trainable_params={_count_trainable_params(fake_model)} "
                f"trainable_filter={os.environ.get('FLASHVSR_STAGE3_FAKE_TRAINABLE_FILTER', 'all')} "
                f"checkpoint={args.stage3_fake_checkpoint} "
                f"attention_mode={args.stage3_fake_attention_mode} "
