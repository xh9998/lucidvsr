...
def _patchify_tokens_with_segments(
    model: WanModel,
    x: torch.Tensor,
    *,
    latent_segment_lengths: Optional[Sequence[Sequence[int]]] = None,
) -> Tuple[torch.Tensor, Tuple[int, int, int]]:
    """
    V5 patchify path.

    When `latent_segment_lengths` is provided, each segment is passed through
    `patch_embedding` independently before token concatenation. This avoids
    temporal mixing across segment boundaries inside Conv3d.
    """
    ...
    for sample_index, one_sample_lengths in enumerate(latent_segment_lengths):
        sample_lengths = [int(length) for length in one_sample_lengths if int(length) > 0]
        offset = 0
        sample_tokens: List[torch.Tensor] = []
        for seg_len in sample_lengths:
            segment = x[sample_index : sample_index + 1, :, offset : offset + seg_len]
            offset += seg_len
            patched = model.patch_embedding(segment)
            cur_f, cur_h, cur_w = patched.shape[2:]
            if patched_h is None:
                patched_f, patched_h, patched_w = cur_f, cur_h, cur_w
            else:
                if (cur_h, cur_w) != (patched_h, patched_w):
                    raise ValueError(
                        f"Patchified segment spatial mismatch: {(cur_h, cur_w)} vs {(patched_h, patched_w)}"
                    )
            sample_tokens.append(rearrange(patched, "b c f h w -> b (f h w) c"))
        if not sample_tokens:
            raise ValueError(f"Sample {sample_index} has no valid latent segments")
        batch_tokens.append(torch.cat(sample_tokens, dim=1))

    if patched_f is None or patched_h is None or patched_w is None:
        raise ValueError("Failed to patchify segmented input")
    max_tokens = max(int(sample.shape[1]) for sample in batch_tokens)
    padded_batch_tokens: List[torch.Tensor] = []
    for sample in batch_tokens:
        pad_tokens = max_tokens - int(sample.shape[1])
        if pad_tokens > 0:
            padding = torch.zeros(
                sample.shape[0],
                pad_tokens,
        ...
        padded_batch_tokens.append(sample)
    max_frames_after_patchify = max(sum(int(v) for v in one_sample) for one_sample in latent_segment_lengths)
    return torch.cat(padded_batch_tokens, dim=0), (max_frames_after_patchify, patched_h, patched_w)


class WanJointModelV5(WanModel):
    """
    ...
    def _patchify_tokens(
        self,
        x: torch.Tensor,
        *,
        latent_segment_lengths: Optional[Sequence[Sequence[int]]] = None,
    ) -> Tuple[torch.Tensor, Tuple[int, int, int]]:
        return _patchify_tokens_with_segments(self, x, latent_segment_lengths=latent_segment_lengths)
    ...
    def forward(
        ...
        **kwargs,
        ...
        x, (f, h, w) = self._patchify_tokens(x, latent_segment_lengths=segment_lengths)
...
def build_joint_wan_from_existing_v5(dit: WanModel) -> WanJointModelV5:
