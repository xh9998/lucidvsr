...
_FLASH_ATTN_BRANCH_REPORTED = set()


def _write_flash_debug_marker(message: str) -> None:
    debug_dir = os.environ.get("FLASHVSR_DEBUG_DIR")
    if not debug_dir:
        return
    os.makedirs(debug_dir, exist_ok=True)
    with open(os.path.join(debug_dir, "flash_attention_branches.log"), "a", encoding="utf-8") as file:
        file.write(message + "\n")
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, compatibility_mode=False):
    ...
    if compatibility_mode:
        ...
        x = F.scaled_dot_product_attention(q, k, v)
        x = rearrange(x, "b n s d -> b s (n d)", n=num_heads)
    report_key = (branch, q.device.type, q.dtype, num_heads)
    if report_key not in _FLASH_ATTN_BRANCH_REPORTED:
        _FLASH_ATTN_BRANCH_REPORTED.add(report_key)
        rank = os.environ.get("RANK", "?")
        local_rank = os.environ.get("LOCAL_RANK", "?")
        message = (
            f"[flash_attention] rank={rank} local_rank={local_rank} "
            f"branch={branch} device={q.device} dtype={q.dtype} "
            f"shape={tuple(q.shape)} num_heads={num_heads}"
        )
        if _flashvsr_train_debug_enabled():
            print(message, flush=True)
        _write_flash_debug_marker(message)
    return x
